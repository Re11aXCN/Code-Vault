...
struct Matrix2D
{
  ...
  constexpr Matrix2D(std::size_t a, std::size_t b, std::size_t c, std::size_t d)
      : data { a, b, c, d }
  {
  }
  ...
  {
    return Matrix2D(data [0] + other.data [0], data [1] + other.data [1],
                    data [2] + other.data [2], data [3] + other.data [3]);
  }

  constexpr Matrix2D operator+ (const std::size_t& scalar) const
  {
    return Matrix2D(data [0] + scalar, data [1] + scalar, data [2] + scalar,
                    data [3] + scalar);
  }
  ...
  {
    return Matrix2D(data [0] * other.data [0] + data [1] * other.data [2],
  ...
  {
    /*
    Cannot write like this: self-assignment (temp *= temp) leads to incorrect
    results because temp is modified during computation, meaning other.data is
    also modified. auto [a, b, c, d] = data; data[0] = a * other.data[0] + b *
    other.data[2]; data[1] = a * other.data[1] + b * other.data[3]; data[2] = c
    * other.data[0] + d * other.data[2]; data[3] = c * other.data[1] + d *
    other.data[3];
    */
  ...
  static constexpr auto matrixPower(const Matrix2D& base, std::size_t exponent)
      -> Matrix2D
  {
    Matrix2D    result;
    Matrix2D    temp = base;
...
inline constexpr auto [[nodiscard]] catalan(std::size_t n) -> std::size_t
{
  ...
  std::size_t result = 1;
  // Use iterative formula: C(n) = (2n)! / (n!(n+1)!)
  // Equivalent to: C(n) = ∏(i=1 to n) (4i-2)/(i+1)
...
// Allow reciprocal usage (use multiplication instead of division)
__attribute__((optimize("-O3 -freciprocal-math")))

// Single precision floating-point optimization
__attribute__((optimize("-O3 -fsingle-precision-constant")))

// More aggressive math optimizations
...
std::size_t catalan_tgamma(std::size_t n)
{
  ...
  // Use gamma function to calculate binomial coefficient C(2n, n)
  double comb =
      std::tgamma(2 * n + 1) / (std::tgamma(n + 1) * std::tgamma(n + 1));

  // Catalan number formula: C(n) = C(2n, n) / (n + 1)
  return comb / (n + 1.0) + 0.1;
}

// Only computes first 15 exact numbers
std::size_t catalan_tgammaf(std::size_t n)
{
  if (n == 0) return 1;
  float comb =
      std::tgammaf(2 * n + 1) / (std::tgammaf(n + 1) * std::tgammaf(n + 1));
...
std::size_t catalan_lgamma(std::size_t n)
{
  ...
  // Use log gamma function to avoid large number overflow
...
// Only computes first 12 exact numbers

...
void _Build_kmp_failure_table(_Iter_diff_t<_RanItPat>* const _Failure,
                              const _RanItPat                _Pat_first,
                              const _Iter_diff_t<_RanItPat>  _Pat_size,
                              _Pred_eq&                      _Eq)
{
  // Builds KMP failure function table from a pattern [_Pat_first, _Pat_first +
  // _Pat_size)
  using _Diff = _Iter_diff_t<_RanItPat>;

  if (_Pat_size == 0) { return; }
  ...
  for (_Diff _Idx = 1; _Idx < _Pat_size; ++_Idx) {
    ...
    if (_Eq(_Pat_first [_Idx], _Pat_first [_Len])) { ++_Len; }
...
pair<_RanItHaystack, _RanItHaystack>
_Kmp_search(const _Iter_diff_t<_RanItPat>* _Failure, const _RanItPat _Pat_first,
            const _Iter_diff_t<_RanItPat> _Pat_size, _RanItHaystack _First,
            _RanItHaystack _Last, _Pred_eq& _Eq)
{
  // KMP string search algorithm
  static_assert(
      is_same_v<_Iter_value_t<_RanItPat>, _Iter_value_t<_RanItHaystack>>,
  ...
  if (_Pat_size == 0) { return { _First, _First }; }
...
public:
  ...
  kmp_searcher(const _RanItPat _First, const _RanItPat _Last,
               _Pred_eq _Eq = _Pred_eq())
      : _Pat_first(_First)
      , _Pat_size(_STD _Get_unwrapped(_Last) - _STD _Get_unwrapped(_First))
      , _Eq(_STD move(_Eq))
  {
    ...
    if (_Pat_size > 0) {
      _Failure_table = _STD make_unique<_Iter_diff_t<_RanItPat>[]>(
          static_cast<size_t>(_Pat_size));
      _STD _Build_kmp_failure_table(
          _Failure_table.get(), _STD _Get_unwrapped(_First), _Pat_size, _Eq);
    }
  }

  template<class _RanItHaystack>
  _NODISCARD pair<_RanItHaystack, _RanItHaystack>
  operator() (const _RanItHaystack _First, const _RanItHaystack _Last) const
  {
    // search for the preprocessed pattern in [_First, _Last)
    if (_Pat_size == 0) { return { _First, _First }; }

    return _STD _Kmp_search(_Failure_table.get(), _Pat_first, _Pat_size, _First,
                            _Last, _Eq);
  }
...
struct _Rabin_karp_hash
{
  ...
  static _Hash_t _Roll_hash(_Hash_t _Old_hash, _Value_t _Old_char,
                            _Value_t _New_char, _Hash_t _Highest_power)
  {
    // Remove old character and add new character
    _Hash_t _New_hash =
        (_Old_hash + _Mod -
         (_To_unsigned_byte(_Old_char) * _Highest_power) % _Mod) %
        _Mod;
  ...
  static _Hash_t _Roll_hash(_Hash_t _Old_hash, const _Value_t* _Old_chars,
                            const _Value_t* _New_chars, size_t _Char_len,
                            _Hash_t _Highest_power)
  {
    ...
    _Hash_t _New_hash =
        (_Old_hash + _Mod - (_Char_value * _Highest_power) % _Mod) % _Mod;
...
template<class _RanItHaystack, class _RanItPat, class _Pred_eq>
pair<_RanItHaystack, _RanItHaystack>
_Rabin_karp_search(const _RanItPat               _Pat_first,
                   const _Iter_diff_t<_RanItPat> _Pat_size,
                   _RanItHaystack _First, _RanItHaystack _Last, _Pred_eq& _Eq)
{
  ...
  const auto _Pattern_hash =
      _Hash::_Compute_hash(_UPat_first, static_cast<size_t>(_Pat_size));

  // Compute power for rolling hash
  const auto _Highest_power =
      _Hash::_Compute_highest_power(static_cast<size_t>(_Pat_size));

  // Compute initial window hash
  auto _Text_hash =
      _Hash::_Compute_hash(_UFirst, static_cast<size_t>(_Pat_size));

  // Check first window
  if (_Text_hash == _Pattern_hash) {
    if (_STD equal(_UPat_first, _UPat_first + _Pat_size, _UFirst,
                   _STD _Pass_fn(_Eq))) {
  ...
  for (auto _Text_start = _UFirst + 1; _Text_start <= _ULast - _Pat_size;
       ++_Text_start) {
    _Text_hash =
        _Hash::_Roll_hash(_Text_hash, *(_Text_start - 1),
                          *(_Text_start + _Pat_size - 1), _Highest_power);

    if (_Text_hash == _Pattern_hash) {
      if (_STD equal(_UPat_first, _UPat_first + _Pat_size, _Text_start,
                     _STD _Pass_fn(_Eq))) {
...
public:
  ...
  rabin_karp_searcher(const _RanItPat _First, const _RanItPat _Last,
                      _Pred_eq _Eq = _Pred_eq())
      : _Pat_first(_First)
      , _Pat_size(_STD _Get_unwrapped(_Last) - _STD _Get_unwrapped(_First))
      , _Eq(_STD move(_Eq))
...
struct _Sunday_bad_char_table
{
  ...
  static void _Build_table(_Iter_diff_t<_RanItPat>*      _Table,
                           const _RanItPat               _Pat_first,
                           const _Iter_diff_t<_RanItPat> _Pat_size)
  {
...
template<class _RanItHaystack, class _RanItPat, class _Pred_eq>
pair<_RanItHaystack, _RanItHaystack>
_Sunday_search(const _RanItPat               _Pat_first,
               const _Iter_diff_t<_RanItPat> _Pat_size, _RanItHaystack _First,
               _RanItHaystack _Last, _Pred_eq& _Eq)
{
  ...
  _Sunday_bad_char_table<_Value_t>::_Build_table(_Bad_char_table, _UPat_first,
                                                 _Pat_size);
  ...
  while (_Text_pos <= _ULast - _Pat_size) {
    ...
    _Diff _Match_pos = 0;
    while (
        _Match_pos < _Pat_size &&
        _STD _Pass_fn(_Eq)(_UPat_first [_Match_pos], _Text_pos [_Match_pos])) {
      ++_Match_pos;
    }
...
public:
  ...
  sunday_searcher(const _RanItPat _First, const _RanItPat _Last,
                  _Pred_eq _Eq = _Pred_eq())
      : _Pat_first(_First)
      , _Pat_size(_STD _Get_unwrapped(_Last) - _STD _Get_unwrapped(_First))
      , _Eq(_STD move(_Eq))

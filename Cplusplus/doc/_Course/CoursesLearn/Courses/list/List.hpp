...
#else
#define DEBUG_INIT_DEADBEAF(T) {(T *)0xdeadbeaf}	// 给指针赋值，如果debug还是这个值，说明没初始化
#endif
...
private:
	...
	ListNode m_dummy;   // 最后一个伪结点，用于end()判断m_head->m_prev,如果m_head为空就会报错，所以需要dummy
	size_t m_size;
...
private:
	// input_iterator 最简单的迭代器，但是效率高些，random有 it+=n 会慢
	template <std::input_iterator InputIt>

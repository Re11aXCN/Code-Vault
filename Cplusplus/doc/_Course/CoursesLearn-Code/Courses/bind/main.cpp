...
int main() {
	...
	Input input;
	Bar bar;
	// pbar是结构体内的变量，&bar是捕获的bar
	input.callback = [pbar = &bar + 1](int i){
			(pbar - 1)->on_input(i);
		};
	// &bar是结构体内的变量，&bar是捕获的bar
	input.callback = [&bar = bar](int i){
			baron_input(i);
		};
	// 如果变量名一致等价于，简写
	// 如果只写&也只会捕获bar，用到哪一个就会捕获哪一个
	input.callback = [&bar](int i){
			baron_input(i);
		};
	bar.age = 43;
	input.main_loop();

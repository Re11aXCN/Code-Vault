...
// fatal error C1075 : “{ ”: 未找到匹配令牌https://blog.csdn.net/jacke121/article/details/108691205
// https://blog.csdn.net/salutlu/article/details/39082373
template <class FnSig>
struct Function {
	// 只在使用了不符合 Ret(Args...) 模式的 FnSig 时出错，惰性编译
...
{
private:
	struct FuncBase { //为了让儿子类也析构需要定义virtual
		// 虚函数同时进行类型擦除

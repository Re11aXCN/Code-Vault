...
* 实际上，`std::function` 本身是支持移动构造和移动赋值的。但是，这并不解决存储仅可移动类型的问题。

* `std::function` 要求存储的可调用对象必须是可复制的。这意味着，如果可调用对象是仅可移动的（例如，捕获了 `std::unique_ptr` 的 lambda），则无法用 `std::function` 来存储。

* 虽然 `std::function` 本身是可移动的，但它要求存储的可调用对象也必须是可复制的，因为 `std::function` 在内部可能需要复制（例如，当需要调整存储空间时）
...
int main() {
    // std::function 要求可调用对象必须是 CopyConstructible
    // 下面代码编译错误，因为lambda捕获了unique_ptr，不可复制
    std::unique_ptr<int> ptr = std::make_unique<int>(42);
    
    // 错误！std::function需要可复制
    // std::function<int()> f = [p = std::move(ptr)]() { return *p; };
    
    // 正确：使用move_only_function
...
std::function<void()> create_func() {
    ...
    // std::function的构造函数会尝试复制lambda，
    // 但lambda捕获的unique_ptr不可复制，导致编译失败
}
```

### 3. std::move_only_function 的使用

随着移动语义在 C++ 中的广泛使用，很多资源管理类**（如 `std::unique_ptr`、`std::thread` 等）**都是仅可移动的。我们经常需要将这些资源与可调用对象结合，例如在异步编程中。

因为`function`如果捕获了上述不能移动的资源管理类，那么将会报错，所以引入仅移动的`std::move_only_function`
...
int main() {
    // 创建一个仅可移动的函数对象，例如捕获了 unique_ptr 的 lambda
    auto unique_lambda = [ptr = std::make_unique<int>(42)]() {
        return *ptr;
    };

    // 使用 std::move_only_function 来包装这个仅可移动的 lambda
    std::move_only_function<int()> f = std::move(unique_lambda);

    // 调用
    std::cout << f() << std::endl; // 输出 42

    // 注意：unique_lambda 已经被移动了，不能再使用

    // 同样，std::move_only_function 本身也是仅可移动的
    auto f2 = std::move(f); // 正确：移动构造
    // auto f3 = f; // 错误：拷贝构造被禁止
    std::cout << f2() << std::endl; // 输出 42
...
使用 std::move_only_function 的情况：

- 需要捕获 `std::unique_ptr`、`std::thread` 等仅移动类型
- 明确禁止复制，保证资源独占
- 需要移动语义优化性能
- 实现仅移动的回调或任务系统
...
### (2) `<any>` 和 [`<variant>`](https://www.bilibili.com/video/BV1f3CXYtE4x/) 对比
...
## [3. `<optional>` 标准库详解](https://github.com/parallel101/stl1weekend/blob/main/Optional.hpp)

`<optional>` 头文件提供了 `std::optional` 类模板，用于表示可能包含值也可能不包含值的包装器。它是处理可能缺失的值的类型安全方式，避免了使用特殊值（如 `nullptr`、`-1` 等）或额外的布尔标志来表示值的存在与否。

```
nullopt_t nullopt; // 一个tag空类，设置为explicit防止多个tag找不到重载，强类型
...
option(nullopt_t) : has_value(fasle), value() {} // 出错因为 Student没有默认构造，value()错误，引入union之后
可以不用写，转而使用
option(nullopt_t) : has_value(fasle) {}// 等到需要的时候我们才进行初始化value

// union的成员初始化时placement new（属于自动对象存储周期，可能在栈可能在堆取决于你是否new使用optional），不分配内存，只构造对象，析构需要手动~T();

先构造成功在设置为true，如果先设置为true，没有构造成功意味着optional对象将永远不会析构
template<class... Ts>
void emplace(Ts&& ...value_args)
{
  if (m_has_value) {
    m_value.~T();
    m_has_value = false;
    new (&m_vaue) T(std::forward<Ts>(value_args)...);
    m_has_value = true;
}
~optional()
{
...
std::optional<C> o2(std::in_place,1,2); //就地构造
std::optional<C> o3(C(1,2));//构造+移动
```

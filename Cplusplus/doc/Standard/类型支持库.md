...
## std::decay_t：类型退化机制详解

`std::decay_t` 是 C++14 引入的类型特性，用于模拟按值传递参数时的类型转换行为。它会移除类型的引用限定符和cv限定符（const/volatile），并将数组和函数类型转换为相应的指针类型。

### 退化规则详解

T& 会 退化为 T ，auto引用退化为普通变量
...
防止T f() 退化 使用 T (&f)()，或者使用标准库 std::function<T()>
```
...
using T3 = std::decay_t<volatile int&&>; // int
using T4 = std::decay_t<const char*>;   // const char* (指针本身不变)
```
...
using T2 = std::decay_t<const char[5]>;  // const char*

// 多维数组
using T3 = std::decay_t<int[3][4]>;      // int(*)[4] (指向一维数组的指针)

// 字符串字面量
const char* str = "hello";  // 字符串字面量会退化为 const char*
```

#### 3. 函数到指针的退化
```cpp
using T1 = std::decay_t<int()>;          // int(*)()
using T2 = std::decay_t<double(int, char)>;  // double(*)(int, char)

// 函数引用保持为函数类型，不退化
using T3 = int(&)();
using T4 = std::decay_t<T3>;            // int(*)()
```
...
int main() {
    ...
    process(arr);   // T 推导为 int*, DecayedT 为 int*
    process(ref);   // T 推导为 int, DecayedT 为 int
}
```
...
// 通用包装器，使用 decay_t 确保存储类型是值类型
template<typename T>
class Wrapper {
private:
    std::decay_t<T> value;  // 存储退化后的类型
...
using tuple_element_decay_t = 
    std::decay_t<typename std::tuple_element<I, Tuple>::type>;

// 示例使用
std::tuple<const int&, double[3], int()> t;
using T1 = tuple_element_decay_t<0, decltype(t)>;  // int
using T2 = tuple_element_decay_t<1, decltype(t)>;  // double*
using T3 = tuple_element_decay_t<2, decltype(t)>;  // int(*)()
```
...
```cpp
template<typename T>
void no_decay_by_reference(T& param) {
    // T 保持为原始数组类型，例如 int[5]
    // param 是引用类型，不会退化
    static_assert(!std::is_same_v<decltype(param), int*>, 
                  "param is not decayed to pointer");
}

int arr[5] = {1, 2, 3, 4, 5};
no_decay_by_reference(arr);  // T 推导为 int[5]
```
...
void process_function(std::function<int(int)> func) {
    // 可以存储函数对象、lambda 或函数指针
    int result = func(42);
}

int add_one(int x) { return x + 1; }

process_function(std::ref(add_one));  // 包装函数指针
```
...
void process_array_ref(T (&arr)[N]) {
    std::cout << "Array size: " << N << std::endl;
    std::cout << "Element size: " << sizeof(T) << std::endl;
    std::cout << "Total size: " << sizeof(arr) << std::endl;
}

// 方法2：使用 std::array（C++11 后推荐）
template<typename T, std::size_t N>
void process_std_array(const std::array<T, N>& arr) {
    std::cout << "Array size: " << N << std::endl;
    std::cout << "Total size: " << sizeof(T) * N << std::endl;
}

// 方法3：使用 Span 类（类似示例中的 Span）
template<typename T>
class Span {
private:
    T* data_;
...
int main() {
    ...
    process_std_array(std_arr);
    
    // 使用 Span
    Span<int> span(arr);  // 自动推导出大小为 10
...
class FunctionRefWrapper {
    Func& func_;  // 保持函数引用
    
public:
    FunctionRefWrapper(Func& func) : func_(func) {}
...
// 方法2：使用 std::function（更灵活，但可能有开销）
class FunctionWrapper {
    std::function<int(int, int)> func_;
    
public:
    FunctionWrapper(std::function<int(int, int)> func) : func_(func) {}
...
// 方法3：使用 auto 返回类型和完美转发
auto make_function_caller = [](auto&& func) {
    // func 通过引用捕获，保持原始类型
...
int main() {
    // 使用函数引用包装器
    FunctionRefWrapper<decltype(add)> ref_wrapper(add);
    std::cout << "5 + 3 = " << ref_wrapper(5, 3) << std::endl;
    
    // 使用 std::function
    FunctionWrapper func_wrapper(add);
    std::cout << "5 * 3 = " << func_wrapper(5, 3) << std::endl;
    
    // 使用 lambda 包装器
    auto caller = make_function_caller(add);
    std::cout << "5 + 3 = " << caller(5, 3) << std::endl;
}
```
...
template<typename T>
class Container {
    std::decay_t<T> value;  // 确保存储的是值类型，而非引用
    
public:
    Container(T&& val) : value(std::forward<T>(val)) {}
};
```
...
constexpr bool is_same_decay_v = 
    std::is_same_v<std::decay_t<T>, std::decay_t<U>>;

static_assert(is_same_decay_v<const int&, int>);      // true
static_assert(is_same_decay_v<int[5], int*>);         // true
static_assert(is_same_decay_v<int(), int(*)()>);      // true
```
...
void register_callback(F&& f) {
    ...
    } else if constexpr (std::is_pointer_v<FuncType> &&
                         std::is_function_v<std::remove_pointer_t<FuncType>>) {
        // 处理函数指针类型
        std::cout << "Function pointer type\n";
    } else {
        // 处理函数对象
        std::cout << "Function object type\n";
    }
}
```

...
### 功能说明
`constexpr` 用于声明可以在编译时求值的表达式、变量或函数。它既可以用于编译期上下文，也可以用于运行期上下文。（注意`constexpr` 不保证编译时求值）
...
视频介绍：https://www.bilibili.com/video/BV11r1FBWE9B/
...
## 使用reinterpret_cast场景而不是static_cast

1. **指针类型之间的转换**：当需要将一种类型的指针转换为另一种类型的指针，而这两种类型之间没有继承关系或者不是同一种类型的变体（如`void*`）时，通常使用`reinterpret_cast`。例如，将一个整数指针转换为一个字符指针。

   ```C++
   // 不同类型指针之间的转换（无继承关系）
   int* p = new int(42);
   char* ch = reinterpret_cast<char*>(p);  // ✓ 必须用 reinterpret_cast
   // char* ch = static_cast<char*>(p);    // ✗ 编译错误
   ```

2. **整数与指针之间的转换**：当需要将整数类型转换为指针类型，或者将指针类型转换为整数类型时，必须使用`reinterpret_cast`。这种转换通常用于低级编程，如操作系统或嵌入式系统编程，其中需要直接处理内存地址。

   ```C++
   uintptr_t address = reinterpret_cast<uintptr_t>(ptr);
   // uintptr_t address = static_cast<uintptr_t>(ptr);  // ✗ 编译错误
   
   void* ptr = reinterpret_cast<void*>(0x12345678);
   ```

3. **函数指针之间的转换**：当需要将一种函数指针转换为另一种函数指针类型时，如果这两种函数指针类型不兼容，则必须使用`reinterpret_cast`。注意，这种转换可能是不安全的，因为不同的函数类型可能有不同的调用约定。

   ```C++
   // 函数指针之间的转换
   void func() {}
   void (*func_ptr)() = func;
   int (*int_func_ptr)() = reinterpret_cast<int(*)()>(func_ptr);
   ```

4. **引用类型的转换**：与指针类似，当需要将一种类型的引用转换为另一种类型的引用，且这两种类型之间没有继承关系时，必须使用`reinterpret_cast`。

   ```C++
   int x = 42;
   float& fref = reinterpret_cast<float&>(x);  // 重新解释位模式
   // float& fref = static_cast<float&>(x);    // ✗ 编译错误
   ```

5. **在多重继承中进行指针调整**：在多重继承中，有时需要将指向派生类的指针转换为指向某个基类的指针，而这两个基类之间没有直接关系，或者需要调整指针的偏移量。这种情况下，`static_cast`会自动调整指针偏移，而`reinterpret_cast`则不会。但是，如果确实需要不调整偏移的原始指针转换（即不改变指针的数值），则必须使用`reinterpret_cast`。注意，这通常只在你知道自己在做什么的情况下使用。
   ...
   // 从B*转回C*时，static_cast会自动调整偏移
   C* pc1 = static_cast<C*>(b);  // ✓ 正确调整偏移
   
   // 但从B*转到A*（不通过C*）时
   A* pa = reinterpret_cast<A*>(b);  // 必须用reinterpret_cast
   // A* pa = static_cast<A*>(b);    // ✗ 编译错误
   ```

6. **类型双关**：当需要将一种类型的对象的内存表示解释为另一种类型时，可以使用`reinterpret_cast`。例如，将一个`float`的位模式解释为`int`。但是，注意这种操作可能违反严格别名规则，导致未定义行为。在某些情况下，可以使用`std::memcpy`来避免别名问题。
   ...
   // 使用 reinterpret_cast 进行类型双关
   float f = 3.14f;
   int i = reinterpret_cast<int&>(f);  // 重新解释位模式
   ```
...
7. **处理内存布局已知的结构**：
   ...
   EthernetHeader* header = reinterpret_cast<EthernetHeader*>(packet);
   ```
...
| ------------ | -------------------- | -------------------- |
| **类型检查** | 编译时检查类型关系   | 几乎不做检查         |
| **指针偏移** | 在继承层次中调整偏移 | 不调整偏移           |
| **安全性**   | 相对安全             | 危险，可能UB         |
| **使用场景** | 相关类型的转换       | 不相关类型的重新解释 |
...
如果预期不会抛出异常，则不能假定程序能够处理该错误，应尽快终止程序。将函数声明为 `noexcept` 通过减少备用执行路径的数量来帮助优化器。它还可以加快失败后的退出速度。

**C++ 标准库对 C 标准库中的所有函数都隐式执行此操作。**

##### 注意

`constexpr` 函数在运行时求值时可能会抛出异常，因此您可能需要为其中一些函数使用条件 `noexcept`。

**析构函数、`swap` 函数、移动操作和默认构造函数永远不应抛出异常**

**另外标准库的std::hash自定义hash也应该声明为noexcept**

对于基虚函数和公共接口的一部分函数，必须谨慎，因为声明一个函数为 `noexcept` 是在建立一个当前和未来实现都必须遵守的保证。对于虚函数，所有重写者也必须是 `noexcept`，并且从函数中删除 `noexcept` 可能会破坏调用函数。
...
首先要明确：`sizeof` 是编译期运算符，它计算的是**变量 / 类型的静态类型大小**。

```C++
class MyClass {
    int a;       // 4字节
    double b;    // 8字节
    char c;      // 1字节（因内存对齐，实际占用8字节）
};
sizeof(MyClass); // 24字节，因为8字节对齐
MyClass* new_obj = nullptr;
sizeof(*new_obj);	// 这里写解引用空指针没问题，编译期间计算，24字节

int* v = new int(10);
constexpr auto vByteSize = sizeof(int) * 10; // 40字节
```

## sizeof 和 std::size 区别

`sizeof`计算字节大小，`std::size`计算原生数组或者标准库容器的长度

```C++
double V[]{1.0,2.0,3.0};
constexpr auto byteSize = sizeof(V); // 24字节
constexpr auto length = std::size(V); // 长度3
```

## 指针退化，导致sizeof计算大小为指针大小

指针的静态类型就是指针，所以 `sizeof(指针变量)` 永远是指针的大小（32位系统4字节，64位系统8字节）。要获取对象的真实大小，核心思路是**绕过指针的静态类型，直接针对对象本身计算**，具体分两种场景处理：

#### 场景1：已知对象的具体类型（非多态场景）
如果明确知道指针指向的对象类型，直接用 `sizeof(对象类型)` 即可，或先解引用指针再计算（需确保指针非空）。
...
int main() {
    ...
    // 错误方式：计算指针本身的大小（64位系统输出8，32位输出4）
    cout << "sizeof(指针) = " << sizeof(ptr) << " 字节" << endl;

    // 正确方式1：直接用类型计算（推荐，无需解引用，更安全）
    cout << "sizeof(MyClass) = " << sizeof(MyClass) << " 字节" << endl; // 24字节，8字节对齐

    // 正确方式2：解引用指针后计算（需确保ptr非空，否则编译没问题但逻辑风险高）
...
#### 场景2：多态场景（基类指针指向派生类对象）
如果是基类指针指向派生类对象，`sizeof(*基类指针)` 仍会计算基类的大小（静态类型），此时无法通过 `sizeof` 直接获取派生类对象的真实大小，需通过**自定义成员函数**解决：
...
public:
    ...
    double derivedData; // 8字节
};

int main() {
    Base* ptr = new Derived();

    // 错误：sizeof(*ptr) 计算的是基类Base的大小（4字节，内存对齐后可能8字节）
    cout << "sizeof(*ptr) = " << sizeof(*ptr) << " 字节" << endl;

    // 正确：通过虚函数获取派生类对象的真实大小（12字节，内存对齐后16字节）
    cout << "ptr->getSize() = " << ptr->getSize() << " 字节" << endl;
...
1. 假设处理器总线宽度是 64 位（8 字节），访问一个未对齐的 `double` 类型（比如起始地址是 0x05），处理器需要做哪些额外操作？

   > 64 位总线访问 0x05 起始的 `double`（8 字节）时，处理器需要**两次内存访问**：先读 0x00-0x07，截取 0x05-0x07；再读 0x08-0x0F，截取 0x08-0x0C；最后拼接成完整的 8 字节 `double`—— 这是性能下降的核心原因

2. 有些架构（如 x86）允许非对齐访问但性能下降，而有些架构（如 ARM）直接禁止非对齐访问并抛出异常 ——C++ 标准对 “非对齐内存访问” 的行为是如何定义的？

   > C++ 标准**并未定义非对齐访问的行为**（属于 “未定义行为”），编译器 / 架构决定是否支持：x86 兼容非对齐但慢，ARMv7+ 需手动开启非对齐访问，否则触发硬件异常

3. 什么是 “自然对齐值（Natural Alignment）”？一个类型的自然对齐值由什么决定？

   > 是**类型的最大基础成员对齐值**（比如结构体的自然对齐值 = 其成员中最大的自然对齐值）；基础类型的自然对齐值 = 其自身大小（如 `double` 是 8，`int` 是 4）

4. 代码中 `alignof(B)` 的结果是多少？为什么 `alignas(4)` 不会生效？

   ```cpp
   // 错误示例：double 自然对齐是 8，alignas(4) 无效
   struct alignas(4) B { double d; };
   
   // GCC/CLANG
   alignas(4) B { double d; } 可以编译通过，但 alignof(B) 结果是 8（而非 4）—— 因为 C++ 标准规定：alignas 指定的值若小于类型的自然对齐值，该指定会被忽略，编译器自动提升到自然对齐值
       
   // MSVC 不能编译
   ```

5. 缓存行（Cache Line）通常是 64 字节，为什么在设计高频访问的结构体（比如线程池的任务结构体、游戏引擎的实体组件）时，要尽量让单个对象大小是缓存行的整数倍，或避免 “缓存行伪共享（False Sharing）”？

   > 是**两个线程修改同一缓存行内的不同变量**，导致缓存行频繁失效（MESI 协议下的缓存一致性开销）

## `alignof`

`alignof`：求一个类型以多少字节对齐，如果是类或者结构体当没有使用`alignas`指定对齐时，以成员变量最大的字节类型对齐，C++最大的基础类型是`long double`16字节

```c++
size_t maxalign = __STDCPP_DEFAULT_NEW_ALIGNMENT__;//最大自然对齐值 16, MSVC/GCC/CLANG都支持
...
`alignas`：让结构体或者类或者数组对齐到多少字节，常用于**placement new**、**缓存行解决伪共享**、**SIMD**对齐
...
Align32 auto* obj = new (raw_mem) MyStruct();
...
// 缓存行，MESI缓存数据一致性，伪共享，blog
// https://zhuanlan.zhihu.com/p/135462276

// 缓存一致性，缓存模型 视频介绍
// https://www.bilibili.com/video/BV1iA4m137Xr/

// C++17 新增硬件缓存行大小，跨平台硬件，64/128等
...
内存对齐地址必须是偶数，否则是UB，因为编译器会优化为SIMD指令，SIMD要求对齐

```cpp
double* p = (double*)malloc(sizeof(double) + sizeof(bool) + sizeof(double));
bool* b = (bool*)(p + 1)
bool* q = (double*)(b + 1) // 未定义错误, 0x5e041fe822b9 奇数，没有对齐
printf("%p\n", q);
```



## 为什么 使用`alignas`后`malloc` 是UB？
...
`malloc` 在C/C++标准中只保证返回**最严格基本类型**的对齐：

- C11标准：`malloc` 返回的指针适合任何具有基本对齐要求的类型
- 基本对齐 = `alignof(max_align_t)`（通常是8或16字节）
- **不保证**能对齐到超过 `alignof(max_align_t)` 的边界

对于你的情况：`alignas(64)` > `alignof(max_align_t)`（通常是16），所以 `malloc` 可能无法提供64字节对齐。

### 2. **UB的具体原因**

- `malloc` 返回的指针可能只对齐到16字节
- 但 `A` 需要64字节对齐
- 将未对齐的指针强制转换为 `A*` 并访问成员是**未定义行为**
...
- `new` 运算符保证返回的指针适合**该类型**的对齐要求
- 即：`new T` 返回的指针对齐到 `alignof(T)`
...
```cpp
// 编译器生成的代码类似于：
void* mem = operator new(sizeof(A), std::align_val_t{alignof(A)});
...
```cpp
#include <cstdlib>
// MSVC是_aligned_malloc和_aligned_free
A* p = static_cast<A*>(_aligned_malloc(sizeof(A), 64));
if (p) {
    // 使用 p
    _aligned_free(p);
}
...
// GCC/CLANG
A* p = static_cast<A*>(std::aligned_alloc(64, sizeof(A))); ，
if (p) {
    std::free(p);
}

// 或者使用 operator new 的重载版本
A* q = new (std::align_val_t{64}) A();
delete q;  // 需要对应的 delete
```

### 2. **C++11/14使用 std::aligned_storage**
...
A* p = new(&storage) A();  // placement new
p->~A();  // 手动析构
```
...
## 使用lambda进行复杂初始化，特别是`const`变量的初始化

##### 原因

它很好地封装了本地初始化，包括清理仅用于初始化的临时变量，而无需创建不必要的非本地但不可重用的函数。它也适用于那些应该为`const`但仅在一些初始化工作之后才能`const`的变量。
...
如果可能，将条件减少为一组简单的选择（例如`enum`），并且不要混淆选择和初始化。
...
```C++
typedef void(foo_t)(void); // 这是声明不是定义
foo_t foo; // 声明一个 foo函数签名
...
typedef void(*foo_t)(void); // 这是是定义
foo_t foo; // 定义一个 foo变量，类型是函数指针签名

void foo(void) { // 重定义了 foo_t foo;
  std::cout << "Hello, world!" << std::endl;
}
```
...
```C++
  ...
  public:
    ...
    bool operator== (const Derived& a) const override // 错误，不能重写父类的虚函数，因为函数签名不同，即因为参数类型不同导致不能重写，不符合override
    {
      return Base::operator==(a) && data == a.data;
    }
  };
```
...
// 解决办法一：
	...
	// 语法正确，但语义上有问题
    bool operator== (const Base& a) const override {
        // 需要将a转换为Derived
        const Derived* p = dynamic_cast<const Derived*>(&a);
        if (!p) return false;  // 类型不同，不相等
        
        return Base::operator==(a) && data == p->data;
    }
```

#### 问题

**问题**：这种实现违反了 **Liskov替换原则** 和 **对称性要求**。

```C++
Derived d1, d2;
Base& b = d2;

d1 == b;  // 调用 Derived::operator==(const Base&) ✅
b == d1;  // 调用 Base::operator==(const Base&) ❌
// 不对称！应该相同但结果可能不同


Base* p1 = new Derived;
Base* p2 = new Base;  // 不是Derived

bool equal = (*p1 == *p2);  // 返回false（类型不同）
// 但 p2 == p1 呢？Base::operator==不知道类型信息
```
...
B& b2 = d2;
b2 == d;   // compares name and number, ignores d2's and d's character，实际上b2应该是D类型
```
...
public:
    bool operator==(const Base& other) const {
        const Derived& d1 = derived();
        const Derived& d2 = static_cast<const Derived&>(other);
...
class Derived : public Base<Derived> {
    ...
    friend class Base<Derived>;  // 允许Base访问私有成员
    
    bool equalsImpl(const Derived& other) const {
        return data == other.data;
    }
    
public:
    // 使用基类的operator==
    // 类型安全：只能比较Derived与Derived
};
```

### 解决办法四：**使用typeid和dynamic_cast**
...
protected:
    // 辅助函数供派生类使用
    bool baseEquals(const Base& other) const {
        return name == other.name && number == other.number;
    }
};
...
public:
    bool operator==(const Base& other) const override {
        ...
        return baseEquals(other) && data == d.data;
    }
};

// 使用智能指针确保安全
bool compare(const std::shared_ptr<Base>& a, const std::shared_ptr<Base>& b) {
    return *a == *b;
}
```

### 解决办法五：**现代C++ - 使用variant/visit**
...
{
  obj() { std::println("obj()"); }
  obj(const obj&) { std::println("obj(const obj&)"); }
  obj& operator=(const obj&) { std::println("obj& operator=(const obj&)"); return *this; }
};
...
int main()
{
  ...
  obj c();        // 声明了一个名为c、返回obj类型、无参数的函数，这个是最容易出错的

  obj d = a;      // 拷贝构造
  d     = b;      // 拷贝赋值

  // 引入了拷贝省略（也叫返回值优化 RVO / 临时对象优化）
  auto e = obj(); // 默认构造，纯右值 obj() ，当用一个纯右值（如 obj()）直接初始化同类型变量时，编译器必须省略拷贝 / 移动构造，直接在变量 e 的内存地址上构造临时对象。
  
  obj  f(a);      // 拷贝构造
  obj  g(a);      // 拷贝构造
  auto h = b;     // 拷贝构造，有名字的左值b 赋值给 左值h
  obj(i);         // 默认构造，等价于 obj i; 类似 数组的写法形式 arr[0] 和 0[arr]
  return 0;
}
```

## 不要定义仅初始化数据成员的默认构造函数；改用类内默认成员初始化器
...
如果你是自定义数据类型和结构，那么确实需要处理自赋值，如果仅仅使用标准库的类型，那么标准库已为你处理好。

您可以通过显式测试自赋值来处理自赋值，但通常它更快、更优雅，无需进行此类测试（例如，[使用 `swap`](https://github.isocpp.org.cn/CppCoreGuidelines/CppCoreGuidelines#Rc-swap)）。
...
`std::string` 对自赋值是安全的，`int` 也是安全的。所有成本都由（罕见的）自赋值案例承担。

## 多态类应禁止公共复制/移动

*多态类*是定义或继承至少一个虚函数的类。它很可能被用作具有多态行为的其他派生类的基类。如果它被意外地按值传递，并使用隐式生成的复制构造函数和赋值，我们就会面临切片：只有派生对象的基类部分会被复制，多态行为会被破坏。

如果类没有数据，则 `=delete` 复制/移动函数。否则，将其设为 `protected`。

## 不要在构造函数和析构函数中调用虚函数

从构造函数或析构函数直接或间接调用未实现的纯虚函数会导致未定义的行为

## 注意事项

* 拷贝赋值比拷贝函数多一个步骤，释放原有内存，然后才拷贝数据覆盖，比如std::string

# `构造函数抛异常`

## 核心概念解析
首先明确核心规则：
- 构造函数抛异常 → **对象整体构造失败**，对象本身不会存在，也不会调用该对象的析构函数
- 已完成初始化的**成员变量**（按初始化逆序）、**基类子对象**会自动析构
- 若成员是裸指针（如`new`出来的对象），因指针本身是普通成员会析构，但指向的堆内存不会自动释放 → 内存泄漏
- RAII类型（如`std::unique_ptr`）的成员会自动调用析构函数释放资源，无需手动处理
- 构造函数的`try-catch`语法是C++专门用于捕获构造过程中异常的特殊语法
...
public:
    // 构造函数try-catch语法：try写在初始化列表后、函数体前
    MyClass(int a, int b) try 
        : base_obj(a),          // 基类初始化
          raii_member(b),       // RAII成员（如std::unique_ptr）
          raw_ptr(new int(b))   // 裸指针成员（手动new）
    {
        // 构造函数体：这里的代码抛异常也会进入catch
        if (b < 0) {
            throw std::runtime_error("b不能为负数");
        }
    }
    catch (const std::exception& e) {
        // 捕获构造过程中（初始化列表+函数体）的异常
        std::cerr << "构造MyClass失败：" << e.what() << std::endl;
        
        // 注意：这里不能手动delete raw_ptr！
        // 因为若raw_ptr已构造，后续会自动析构（但指针本身析构不释放堆内存）
        // 若raw_ptr未构造（比如base_obj初始化抛异常），delete会导致未定义行为
        
        // 可以记录日志、清理非对象成员的资源（如文件句柄），但不要处理对象成员
        throw; // 必须重新抛出！否则编译器会自动将异常转为std::bad_exception
    }

private:
    BaseClass base_obj;
    std::unique_ptr<int> raii_member; // RAII成员
    int* raw_ptr; // 裸指针成员
};
...
public:
    BaseClass(int a) {
        if (a > 100) {
            throw std::out_of_range("a超过范围");
        }
        std::cout << "BaseClass构造" << std::endl;
    }
    ~BaseClass() {
        std::cout << "BaseClass析构" << std::endl;
    }
};
```
...
// 上面的BaseClass和MyClass定义...

int main() {
    try {
        // 测试1：基类初始化抛异常（a=200>100）
        MyClass obj1(200, 5);
    } catch (const std::exception& e) {
        std::cerr << "捕获异常：" << e.what() << std::endl;
    }
    // 输出：
    // BaseClass析构（基类已构造，自动析构）
    // 构造MyClass失败：a超过范围
    // 捕获异常：a超过范围
...
class BadClass {
public:
    BadClass() : ptr(new int(10)) {
        throw std::runtime_error("构造失败"); // 抛异常
    }
    ~BadClass() {
        delete ptr; // 不会执行！因为对象构造失败，析构函数不调用
    }
private:
    int* ptr; // 裸指针
};

// 调用后，new int(10)的内存泄漏！
// 因为ptr已构造（指针本身是栈上的），但对象整体构造失败，析构函数不执行，无法delete
```
...
class GoodClass {
public:
    GoodClass() : ptr(std::make_unique<int>(10)) {
        throw std::runtime_error("构造失败"); // 抛异常
    }
    // 无需手动析构！std::unique_ptr会自动处理
private:
    std::unique_ptr<int> ptr; // RAII成员
};

// 调用后，std::unique_ptr的析构函数会自动调用，释放new int(10)的内存
// 因为ptr是已构造的成员，会按逆序自动析构
```

## 构造函数try-catch的使用场景
1. **日志记录**：捕获异常并记录详细的构造失败原因
2. **清理非对象资源**：如构造过程中打开的文件句柄、网络连接（非成员变量的资源）
3. **异常转换**：将底层异常转为更上层的业务异常（需重新抛出）

⚠️ **注意**：
- 构造函数的catch块中**必须重新抛出异常**（`throw;`），否则编译器会自动抛出`std::bad_exception`
- 不要在catch块中手动释放成员变量的资源（如`delete raw_ptr`），可能导致重复释放或未定义行为

## 总结
1. **核心规则**：构造函数抛异常 → 对象未构造完成，自身析构函数不执行，但**已构造的基类、成员变量会按逆序自动析构**。
2. **资源管理**：裸指针成员的堆内存不会自动释放（内存泄漏），而**RAII类型（如std::unique_ptr）会通过自身析构函数自动释放资源**，这是C++资源管理的最佳实践。
3. **try-catch语法**：构造函数的`try-catch`用于捕获构造过程（初始化列表+函数体）的异常，主要用于日志/资源清理，且必须重新抛出异常。

关键点：构造函数异常的核心是“已构造的部分自动析构，未构造的部分无影响”，而RAII是解决构造异常中资源泄漏的根本方案，应优先使用RAII而非裸指针。

# `析构函数`

现代C++除非显示声明析构函数为 noexcept(false)，否则编译器会自动添加noexcept，认为析构函数不抛异常，将确保它要么正常完成，要么终止程序。另外一旦声明析构函数为 noexcept(false)，那么其子、孙类会继承这个特性。
...
“这个类需要析构函数吗？”是一个令人惊讶的富有洞察力的设计问题。对于大多数类来说，答案是“否”，原因要么是类不持有任何资源，要么是析构处理由[零法则](https://github.isocpp.org.cn/CppCoreGuidelines/CppCoreGuidelines#Rc-zero)完成；也就是说，它的成员可以处理好析构方面的事情。如果答案是“是”，那么类的许多设计将随之而来（参见[五法则](https://github.isocpp.org.cn/CppCoreGuidelines/CppCoreGuidelines#Rc-five)）。



# `零法则`/`三五法则`

如果类的成员具有能够自动管理内存的能力，或者你没有特殊需求，那么无需添加默认构造、拷贝构造/赋值、移动构造/赋值，即零法则，如果你确实需要添加，那么请将这5个函数声明为=default

如果类具有拥有指针成员，请定义析构函数，并且明确所有权，delete
...
- **C++**：类（对象）的非静态成员销毁顺序是**声明顺序的逆序**，本质和“栈的后进先出”逻辑一致；
- **Rust**：结构体/枚举的字段销毁顺序是**严格按声明顺序**，这是 Rust 语言的明确规定（无“栈逆序”逻辑）。

---

## C++ 类成员的销毁规则
### 1. 核心规则
- **构造顺序**：按成员**声明的顺序**构造（和初始化列表顺序无关）；
- **销毁顺序**：按成员**声明的逆序**销毁（栈的后进先出特性）。
...
struct Member {
    const char* name;
    Member(const char* n) : name(n) { std::cout << "构造: " << name << std::endl; }
    ~Member() { std::cout << "销毁: " << name << std::endl; }
};
...
### 3. 背后原因
C++ 设计时借鉴了“栈”的逻辑：
- 构造时先初始化的成员（声明在前），相当于“先入栈”；
- 销毁时后初始化的成员（声明在后），需要“先出栈”（逆序），确保依赖关系安全（比如成员 b 依赖成员 a，a 销毁前 b 先销毁，避免悬垂引用）。

---

## 二、Rust 结构体字段的销毁规则
### 1. 核心规则
- **构造/初始化**：按声明顺序初始化（和 C++ 一致）；
- **销毁（Drop）**：按声明顺序销毁（和 C++ 相反），这是 Rust 语言规范的强制要求。

### 2. 代码验证
```rust
struct Member(&'static str);

// 实现 Drop trait 观察销毁顺序
...
### 3. 背后原因
Rust 选择按声明顺序销毁，核心是为了**确定性和安全性**：
- Rust 强调“可预测的资源管理”，按声明顺序销毁能让开发者明确知道字段的销毁时机，避免因逆序导致的逻辑混乱；
- 若字段间有依赖（比如 b 持有 a 的引用），Rust 会通过借用检查器强制要求“被引用的字段声明在前”，因此按顺序销毁时，引用方（b）先销毁，被引用方（a）后销毁，天然避免悬垂引用（无需逆序）。
...
1. 先销毁派生类的成员（逆序）；
2. 再调用派生类析构函数；
3. 最后销毁基类的成员（逆序）→ 基类析构函数。

### 2. Rust 嵌套结构体的销毁
Rust 中，外层结构体的字段销毁时，会先销毁字段本身（按声明顺序），再递归销毁字段内部的成员（同样按声明顺序）。

### 3. Rust 的 Drop 手动实现
如果为结构体手动实现 `Drop` trait，`drop` 方法的执行时机是“字段销毁前”，但字段本身的销毁仍按声明顺序：
```rust
impl Drop for MyStruct {
    fn drop(&mut self) {
        println!("MyStruct 开始销毁"); // 先执行这个
    }
}
// 输出：
// MyStruct 开始销毁
...
**当使用一个函数其参数的内容，在某个命名空间中，根据ADL机制，外部的函数知道从命名空间找，如果找不到在去命名空间的上一层找知道找到全局作用域的层级，没找到就说明失败**
...
std::vector<int> a;
a = move(a); // ok，知道在std中 等于 std::move(a);
...
std::vector<int> a;
a = move(move(a)); // 内部调用move(_Ty& _Arg)，外部调用标准库的，因为内部更匹配全局move，全局move返回了&&又更匹配标准库的move
// 等价于 std::move(::move(a)); 一个从全局找，一个从标准库找

// 如果把全局的move的参数&去掉，那么只能 ::move(std::move(a)); or std::move(::move(a));
// or ::move(::move(a)); or std::move(std::move(a)); 不能省略域作用符号::
```
...
现代C++中函数签名里各种修饰符（比如`&`、`const`、`&&`、`noexcept`）的含义和作用，这些修饰符是C++11及以后版本为了增强函数行为控制、支持移动语义和提升代码安全性而引入的核心特性。

## 核心修饰符分类及作用详解
### 1. `const` 修饰成员函数
**作用**：标记成员函数不会修改对象的**非mutable**成员变量，是C++98就有的特性，但现代C++中与引用限定符结合后更强大。
- 语法：`返回值 类名::func() const`
- 核心规则：
  - `const`**成员函数内不能修改对象的普通成员变量**（除非成员用`mutable`修饰）。
  - **`const`对象、`const`指针/引用指向的对象，只能调用`const`成员函数。**
  - 非`const`对象可以调用`const`或非`const`成员函数（优先调用非`const`版本）。
...
private:
    int value = 10;
    mutable int count = 0; // mutable成员可在const函数中修改
public:
    // const成员函数：只读访问对象
    int getValue() const {
        count++; // 允许，因为count是mutable
        // value++; // 编译错误：不能修改非mutable成员
        return value;
    }
...
int main() {
    const MyClass const_obj; // const对象
    cout << const_obj.getValue() << endl; // 合法：调用const成员函数
    // const_obj.setValue(20); // 编译错误：const对象不能调用非const函数

    MyClass non_const_obj; // 非const对象
    non_const_obj.setValue(20); // 合法
    cout << non_const_obj.getValue() << endl; // 合法
    return 0;
}
```

### 2. 引用限定符（`&` / `&&`）：C++11新增
**作用**：**限制成员函数只能被左值对象（`&`）或右值对象（`&&`）调用**，核心用于区分对象的“左值/右值身份”，配合移动语义优化性能。

- 语法：`返回值 类名::func() &`（左值限定）、`返回值 类名::func() &&`（右值限定）
- 核心规则：
  - `&`修饰的函数：仅左值对象（**有名字、可取地址的对象**）能调用。
  - `&&`修饰的函数：仅右值对象（**临时对象、移动后的对象**）能调用。
  - 引用限定符只能用于**非静态成员函数**。
...
private:
    string str;
public:
    StringWrapper(string s) : str(std::move(s)) {}

    // 左值限定：仅左值对象调用，返回左值引用
    string& getStr() & {
        cout << "调用左值版本getStr()" << endl;
        return str;
    }

    // 右值限定：仅右值对象调用，返回右值引用（支持移动）
    string&& getStr() && {
        cout << "调用右值版本getStr()" << endl;
        return std::move(str);
    }
};

int main() {
    StringWrapper obj("hello"); // 左值对象
    obj.getStr(); // 调用左值版本

    // 临时对象（右值）调用，触发右值版本
    StringWrapper("world").getStr(); 
...
### 3. `const` + 引用限定符组合（`const &` / `const &&`）
**作用**：结合`const`的“只读”特性和引用限定符的“左/右值限制”，是更精细的行为控制。
- `func() const &`：仅**const左值对象**或**非const左值对象**（隐式转换为const）能调用，且函数内只读。
- `func() const &&`：仅**const右值对象**能调用，且函数内只读（实际使用场景较少，主要用于完善语义）。
...
public:
    // const左值限定：只读，仅左值调用
    void print() const & {
        cout << "const左值对象调用print()" << endl;
    }

    // const右值限定：只读，仅右值调用
    void print() const && {
        cout << "const右值对象调用print()" << endl;
    }
};

int main() {
    const MyClass const_obj;
    const_obj.print(); // 调用const &版本

    // const临时对象（右值）调用const &&版本
    const MyClass().print(); 
    return 0;
}
```
...
### 4. `noexcept`：C++11新增
**作用**：标记函数**不会抛出异常**（或明确声明可能抛出的异常），核心价值是：
- 提升代码安全性：编译器可基于`noexcept`做优化（比如移动构造函数加`noexcept`后，STL容器会优先选择移动而非拷贝）。
- 明确异常语义：告诉调用者函数的异常行为，避免未捕获的异常导致程序崩溃。
- 语法：
  - `func() noexcept`：函数绝对不抛出异常。
  - `func() noexcept(表达式)`：表达式为`true`时不抛异常，否则可能抛（比如`noexcept(std::is_nothrow_move_constructible_v<T>)`）。
- 核心规则：
  - 如果`noexcept`函数实际抛出了异常，程序会直接调用`std::terminate()`终止（而非触发异常处理）。
  - 析构函数默认是`noexcept(true)`（除非显式声明为`noexcept(false)`）。
...
// 有条件的noexcept：仅当T的析构函数不抛异常时，该函数才noexcept
template <typename T>
void destroy(T* ptr) noexcept(noexcept(ptr->~T())) {
    ptr->~T();
}

// 违反noexcept声明：会触发terminate
void unsafe_func() noexcept {
    throw runtime_error("意外抛出异常"); // 运行时崩溃
}

int main() {
    safe_func();
    // unsafe_func(); // 取消注释会导致程序终止
    return 0;
}
```
...
| 函数签名               | 适用对象               | 核心特性                     | 典型场景                     |
|------------------------|------------------------|------------------------------|------------------------------|
| `func()`               | 非const左值对象        | 可修改对象，仅左值调用       | 普通成员函数（修改对象状态） |
| `func() const`         | const/非const左值对象  | 只读，仅左值调用             | 获取对象属性（如`getValue()`）|
| `func() &`             | 非const左值对象        | 可修改，仅左值调用           | 左值对象的修改操作           |
| `func() const &`       | 所有左值对象（const/非const） | 只读，仅左值调用         | 左值对象的只读操作           |
| `func() &&`            | 右值对象               | 可修改，仅右值调用           | 移动语义（如临时对象的资源转移） |
| `func() const &&`      | const右值对象          | 只读，仅右值调用             | 少见，const临时对象的只读操作 |
| `func() noexcept`      | 任意对象（视其他修饰符） | 不抛异常                     | 高性能/安全关键函数（如移动构造） |

你想知道除了之前提到的`const`、引用限定符（`&`/`&&`）、`noexcept`之外，现代C++中还有哪些函数签名修饰符，以及它们各自的用途——这些修饰符覆盖了返回值约束、协变/逆变、编译期检查、链接属性等多个维度，是完善函数行为控制的重要补充。

## 返回值相关修饰符
### 1. `[[nodiscard]]`（C++17 新增）
**作用**：强制编译器对“丢弃该函数返回值”的行为发出警告，核心用于避免因疏忽忽略关键返回值（比如错误码、资源句柄）导致的逻辑错误。
- 语法：可修饰函数、返回值类型、枚举/类（修饰类/枚举时，所有返回该类型的函数都会被检查）。
- 核心规则：仅当返回值被**显式丢弃**（比如直接调用函数不接收返回值）时触发警告，若返回值被转换为`void`（如`(void)func()`）则不会警告。
...
ErrorCode open_file(const string& path) {
    return ErrorCode::FileNotFound;
}

int main() {
    divide(10, 2); // 警告：丢弃nodiscard函数的返回值
    open_file("test.txt"); // 警告：丢弃nodiscard类型的返回值
    (void)divide(10, 0); // 无警告：显式丢弃
    return 0;
}
```

### 2. `[[maybe_unused]]`（C++17 新增）
**作用**：告诉编译器“该函数/参数可能未被使用”，抑制编译器的“未使用”警告，核心用于兼容模板、条件编译等场景。
- 语法：可修饰函数、参数、变量、类型等。

**示例**：
```cpp
// 修饰函数：即使未调用也不报警告
[[maybe_unused]] void debug_log(const string& msg) {
#ifdef DEBUG
    cout << "[DEBUG] " << msg << endl;
#endif
}
...
// 修饰参数：即使参数未使用也不报警告
int calculate(int a, [[maybe_unused]] int b) {
    return a * 2; // b未使用，但无警告
}

int main() {
    calculate(10, 20); // 正常运行，无警告
    // debug_log未调用，但无警告
    return 0;
}
```
...
`virtual`  C++98  声明虚函数，支持多态（派生类可重写），现代C++中常与`override`/`final`配合使用

### 2. `override`（C++11 新增）
**作用**：显式声明“该成员函数是重写基类的虚函数”，核心价值是：

- 编译期检查：如果函数签名与基类虚函数不匹配（比如参数、返回值错误），编译器直接报错，避免“意外重载而非重写”的隐蔽错误。
- 增强可读性：一眼看出该函数是重写的基类函数。
- 语法：仅能修饰**虚成员函数**，且必须放在函数签名末尾。
...
public:
    // 正确：重写基类的show（签名完全匹配）
    void show(int x) const override {
        cout << "Derived: " << x << endl;
    }

    // 错误：参数类型不匹配，override会触发编译错误
    // void show(double x) const override {}
};

int main() {
    Base* ptr = new Derived();
    ptr->show(10); // 调用Derived::show，多态生效
    delete ptr;
    return 0;
}
```
...
### 3. `final`（C++11 新增）
**作用**：有两种用法，核心是“禁止重写/继承”：
- 修饰**虚成员函数**：禁止派生类重写该函数。
- 修饰**类**：禁止该类被继承。
- 语法：放在函数签名末尾（修饰函数）或类名后（修饰类）。
...
public:
    virtual void func() final {
        cout << "Base::func 不可被重写" << endl;
    }
};

class Derived : public Base {
public:
    // 错误：func被final修饰，无法重写
    // void func() override {}
};
...
## 链接属性修饰符（C语言兼容/链接控制）
### 1. `extern "C"`
**作用**：指定函数使用C语言的链接规则（而非C++的名字修饰规则），核心用于C++与C代码的交互（比如调用C语言编写的库函数）。
- 语法：可修饰单个函数，也可包裹函数/类声明块。
- 核心规则：
  - C++编译器对`extern "C"`函数不会进行名字修饰（name mangling），因此C代码能通过原始函数名调用。
  - `extern "C"`函数不能是模板函数、不能重载（因为C不支持重载）。

**示例**：
```cpp
// 声明C语言风格的函数
extern "C" void c_style_func() {
    cout << "该函数使用C链接规则" << endl;
}
...
### 2. `static`（函数级）
**作用**：限制函数的链接范围为“当前编译单元（.cpp文件）”，核心用于避免多个编译单元中同名函数的冲突，实现“文件内私有”。
- 语法：放在函数返回值前（修饰全局函数）。
- 核心规则：
  - `static`函数不能被其他编译单元调用（链接时不可见）。
  - C++17后推荐用`namespace`+`[[maybe_unused]]`或匿名命名空间替代全局`static`函数，语义更清晰。
...
int main() {
    private_func(); // 合法：当前编译单元内调用
    return 0;
}
```

## 编译期函数修饰符
### 1. `constexpr`（C++11 新增，C++14/20增强）
**作用**：标记函数为“常量表达式函数”，核心价值是：
- 编译期计算：函数调用可在编译期完成，替代#define宏，提升性能且类型安全。
- 可用于常量上下文：比如数组大小、模板参数、枚举值等。
- 核心规则（C++14后）：
  - 函数体可包含简单分支、循环（C++11仅允许单条return）。
  - 非void返回值的`constexpr`函数，若传入编译期常量，则返回编译期常量；若传入运行时变量，则退化为普通函数。
  - 可修饰成员函数、构造函数（C++11支持constexpr构造函数）。
...
int main() {
    // 编译期计算：数组大小由constexpr函数结果确定
    int arr[square(5)] = {0}; // 数组大小为25
    constexpr int f5 = factorial(5); // 编译期计算得120
    int x = 10;
    int s = square(x); // 运行时计算（x是变量）
    return 0;
}
```

### 2. `consteval`（C++20 新增）
**作用**：比`constexpr`更严格，强制函数“必须在编译期执行”，核心用于确保函数绝对不会在运行时调用，避免意外的运行时开销。
- 语法：放在函数返回值前。
- 核心规则：
  - `consteval`函数的所有参数必须是编译期常量。
  - 调用`consteval`函数必须产生编译期常量，否则直接编译错误（不会退化）。
...
int main() {
    constexpr int res1 = add_consteval(3, 4); // 合法：编译期计算
    // int x = 3;
    // int res2 = add_consteval(x, 4); // 错误：x是运行时变量，无法编译期执行
    return 0;
}
```
...
| 修饰符           | 引入版本 | 核心作用                                                     |
| ---------------- | -------- | ------------------------------------------------------------ |
| `inline`         | C++98    | 建议编译器将函数展开（减少函数调用开销）（现代的C++中编译器会自动内联，仅作为单一定义规则的使用或者语法规范而已），同时放宽“单定义规则”（多编译单元定义同一inline函数不冲突） |
| `[[deprecated]]` | C++14    | 标记函数为“已废弃”，调用时编译器发出警告，可附带提示信息（如`[[deprecated("use new_func() instead")]]`） |
| `explicit`       | C++98    | 仅修饰构造函数/转换函数，禁止隐式类型转换（避免意外的类型转换，提升代码安全性） |

**`[[deprecated]]`示例**：
```cpp
// 标记函数已废弃，并提示替代方案
[[deprecated("use new_print() instead")]]
void old_print(const string& msg) {
    cout << "[OLD] " << msg << endl;
}

void new_print(const string& msg) {
    cout << "[NEW] " << msg << endl;
}

int main() {
    old_print("test"); // 警告：调用已废弃的函数
    new_print("test"); // 正常
    return 0;
}
```
...
你想了解C++中可重载与不可重载的运算符清单，同时还想知道在C++23标准及C++26的提案中，那些原本不可重载的运算符是否有被提议改为可重载的情况。我会先清晰列出这些内容，再结合最新的标准和提案为你说明。

## 一、C++中可重载与不可重载的运算符
### 1. 可重载的运算符（核心清单）
C++允许重载的运算符涵盖大部分常用运算符，分为单目、双目、三目（仅`?:`不可重载）、赋值/复合赋值、类型转换等类别，具体包括：
| 类别         | 运算符示例                                                                 |
|--------------|----------------------------------------------------------------------------|
| 算术运算符   | `+ - * / % ++ --`                                                          |
| 关系运算符   | `== != < > <= >=`                                                          |
| 逻辑运算符   | `! && ||`（注：`&&`/`||`重载后会丢失“短路求值”特性）                        |
| 位运算符     | `& | ^ ~ << >>`                                                            |
| 赋值/复合赋值 | `= += -= *= /= %= &= |= ^= <<= >>=`                                       |
| 单目运算符   | `+ - * & ! ~ ++ --`（前缀/后缀`++`/`--`可分别重载）                        |
| 指针/成员访问 | `-> ->* []`                                                                |
| 调用/类型转换 | `()`（函数调用）、`new new[] delete delete[]`、`static_cast`之外的类型转换 |
| 其他         | `,`（逗号运算符）、`->`（成员访问）                                        |

### 2. 不可重载的运算符（核心清单）
这些运算符因语言设计、安全性或语义不可替代性，**始终不允许重载**，核心包括：
- `.`：成员访问运算符（直接访问类成员）
- `.*`：成员指针访问运算符
- `::`：作用域解析运算符
- `?:`：三目条件运算符（三元运算符）
- `sizeof`：大小计算运算符
- `typeid`：类型识别运算符
- `alignof`（C++11起）：对齐大小运算符
- `noexcept`（C++11起）：异常说明运算符
- `typeof`/`decltype`（关键字，非运算符但常被混淆）

## 二、C++23及C++26提案中关于运算符重载的变化
### 1. C++23标准：无“不可重载→可重载”的运算符变更
C++23主要聚焦于语法简化、库增强（如`std::format`）、协程完善等，**并未修改运算符重载的核心规则**，上述不可重载的运算符在C++23中仍不可重载。

### 2. C++26提案：暂无“开放不可重载运算符”的核心提案
截至2026年最新的WG21（C++标准委员会）提案，**没有任何正式提案提议开放上述不可重载运算符的重载权限**，核心原因：
- 这些运算符（如`.`、`::`、`?:`）是C++语法的“基础骨架”，重载会破坏语言的一致性和可读性；
- 委员会更关注“安全重载”（如限制某些运算符的重载场景），而非开放核心不可重载运算符；
- 少数提案仅涉及“运算符重载的语法优化”（如简化`operator[]`的重载规则），而非新增可重载运算符。

## 补充：易混淆的“伪重载”场景

有些场景看似“重载不可重载运算符”，实则是语法糖或替代方案，并非真正重载：
1. **`.`运算符的替代**：通过`operator->`间接模拟（如智能指针`std::unique_ptr`的`->`），但无法直接重载`.`；
2. **`?:`的替代**：用函数封装（如`template <typename T> T ternary(bool cond, T a, T b) { return cond ? a : b; }`），但丢失短路求值特性；
3. **`sizeof`的扩展**：通过用户定义类型的`sizeof`计算（如`sizeof(std::string)`），但`sizeof`本身仍是不可重载的运算符，只是其计算结果依赖类型的内存布局。

## 总结
1. **核心规则**：C++中`.、.*、::、?:、sizeof、typeid、alignof、noexcept`不可重载，其余大部分运算符可重载；
2. **C++23变化**：未修改运算符重载规则，不可重载的运算符仍不可重载；
3. **C++26提案**：暂无开放不可重载运算符的正式提案，委员会更关注重载的安全性而非扩展可重载范围。

如果你的场景是“想模拟某个不可重载运算符的行为”，可以进一步说明，我会为你提供合规的替代实现方案。
...
int main()
{
  Signal<std::string, std::string> s;
  s.connect(test);                                                   // error
  s.connect(static_cast<void (*)(std::string, std::string)>(test));  // ok
  s.connect(FUN(test));                                              // ok

  struct
  {
    void operator() (int a, std::string b) const { }
...
想弄明白为什么在使用 `std::function` 绑定重载函数 `test` 时会直接报错，但通过强制类型转换、宏定义的 lambda 或者仿函数的方式却能成功，核心是要理解重载函数名在不同场景下的类型推导规则。

在C++中，函数名`test`本身是**重载集（overload set）**，包含两个不同的函数签名：

1. `void(std::string, std::string)`
2. `void(int, std::string)`

test有两个虽然参数不同，但是函数名字都是test不知道你要选哪一个，static_cast确定一个，lambda延迟调用泛型匹配，仿函数只有一个实例对象，实例对象调用自己的仿函数ok

### 1. 直接 `s.connect(test)` 报错的本质
C++ 中，**重载函数名本身没有确定的类型**，它只是一组具有不同签名的函数的“名称集合”。当你直接把 `test` 传给 `connect` 函数（参数是 `std::function<void(std::string, std::string)>`）时：
- 编译器需要先确定 `test` 的具体类型（即要选哪个重载版本），才能将其转换为 `std::function`；
- 但 `std::function` 的构造是一个**隐式类型转换过程**，这个过程无法反向推导“应该选哪个重载函数”，编译器陷入二义性，因此直接报错。

简单说：编译器在**模板实例化阶段**就需要知道确切的函数类型。重载集`test`没有明确的类型，因此编译失败。编译器看到 `test` 时，不知道你要选 `test(std::string, std::string)` 还是 `test(int, std::string)`，而 `std::function` 无法帮它做这个选择。

### 2. `static_cast` 强制转换可行的原因
`static_cast<void(*)(std::string, std::string)>(test)` 显式告诉编译器：

- 我要选的是**签名为 `void(std::string, std::string)` 的那个 `test` 函数**；
- 强制转换后，`test` 被确定为一个具体的函数指针，而 `std::function` 可以直接接收函数指针作为构造参数，因此无错。

### 3. 宏定义的 lambda（`FUN(test)`）可行的原因
你的宏 `#define FUN(fun) [&](auto...t) { return fun(t...); }` 展开后是一个**泛型 lambda**：
```cpp
[&](auto... t) { return test(t...); }
```
这个 lambda 的关键特性：

* lambda的参数是`auto...t`，在lambda**内部调用时才进行重载解析**

- 泛型 lambda 的参数类型是**延迟推导**的——只有当 lambda 被调用时（即 Signal 触发回调时），才会根据传入的参数（`std::string, std::string`）去匹配 `test` 的重载版本；
- 传给 `connect` 时，lambda 本身是一个**无重载的、类型确定的可调用对象**（泛型 lambda 的类型是编译器生成的唯一匿名类型），因此编译器不需要立刻解析 `test` 的重载，自然不会报错；
- 当 Signal 调用这个 lambda 时，参数是 `std::string, std::string`，此时编译器能明确匹配到对应的 `test` 重载版本。

### 4. 仿函数（匿名结构体）可行的原因

你的仿函数是一个**具有重载 `operator()` 的类对象**，和普通函数重载的核心区别：
- 仿函数对象是**一个具体的、类型确定的实例**（没有多个“同名对象”，只有一个对象包含多个重载的 `operator()`）；
- 当 `std::function` 包装这个仿函数时，编译器会根据 `std::function` 的签名（`void(std::string, std::string)`）去检查：这个仿函数的 `operator()` 是否有能匹配该签名的版本——`test.operator()(std::string, std::string)`；
- 仿函数的 `operator()` 重载是“属于对象的”，编译器可以基于 `std::function` 的签名筛选出匹配的 `operator()`，因此无二义性。

简单总结：函数重载是“同名不同实体”，仿函数是“一个实体包含多个重载方法”，前者无法直接推导，后者可以。

### 总结
核心关键点回顾：
1. **直接传重载函数名报错**：重载函数名无确定类型，`std::function` 无法推导要选哪个重载版本，导致二义性；
2. **强制类型转换可行**：显式指定函数签名，消除二义性，让编译器确定具体的函数指针；
3. **泛型 lambda 可行**：延迟解析重载（调用时才匹配），lambda 本身是类型确定的可调用对象；
4. **仿函数可行**：仿函数是单个类型确定的对象，编译器可基于 `std::function` 签名筛选匹配的 `operator()`。

本质上，所有可行的方式都是**消除了“重载函数名的二义性”** ——要么显式指定类型，要么延迟解析，要么将重载封装到单个对象中。
# `遮蔽`
...
如果x未被外部使用，则应该使用C++17新特性`if`、`switch` 和范围 `for` 初始化语句
...
或者减小函数体代码量应该封装代码，并且命名规范，尽量避免代码量过长忘记定义了哪些变量名字

# `待决名`

当使用模板编程，使用模板类型参数::type或者包含t的变量时，想要获取类型内的某一个类型时需要加上typename，原因是编译器不知道type是类型还是值，它会假定是值，所有声明typename告诉编译器它是类型

```C++
template<class T>
auto func(T t) {
    if constexpr (typename std::same_as<std::decay<decltype(T)>::type, int>){
    }
    else if constexpr (typename std::same_as<std::decay<decltype(t)>::type, double>){
	}
}
```
...
- **定义**：一种编译器优化技术，**允许编译器在函数返回时，省略调用者处的一个临时对象的创建和拷贝/移动操作**。
- **匿名RVO**：特指返回一个**匿名临时对象**时的优化。例如：`return MyClass(10, 20);`
- **基本原理**：编译器直接在函数调用者为返回值分配的内存位置上构造这个返回对象，省去了“函数内构造 -> 临时对象 -> 拷贝/移动到目标”的步骤。

## 2. **NRVO (Named Return Value Optimization)**

- **定义**：RVO的一个更强大的变体。它允许编译器省略一个**有名字的局部变量**作为返回值时的拷贝/移动操作。
...
- **示例**：
  ...
  MyClass createObject() {
      MyClass obj; // 有名字的局部对象
      obj.process();
      return obj; // NRVO 可能在此发生
  }
  ```

  

- **基本原理**：编译器会尝试在函数调用者为返回值预留的地址上，直接构造这个局部变量 `obj`。这样，`obj` 从出生地就在它最终该在的地方（调用者的栈帧上），`return` 语句不需要进行任何实质性的“搬运”工作。

## 3. 与移动语义的区别与联系

- **联系与互补**：
  - **共同目标**：提升性能，减少不必要的拷贝开销。
  - **执行顺序**：在可能的情况下，**编译器会优先尝试RVO/NRVO**。这是一项**优化**，编译器可以选择不做（但在C++17后，对纯RVO有强制要求）。
  - **降级保障**：如果RVO/NRVO未发生（例如，编译器无法确定返回哪个变量，或优化被关闭），函数返回会**退而求其次，尝试使用移动语义**（如果返回的局部对象类型有移动构造函数）。如果移动也不行，则最后使用拷贝。
  - **移动语义是“语言标准保障的优化”**，而NRVO是“编译器实现的更强优化”。

* **本质区别**：

| 特性         | NRVO/RVO                                               | 移动语义                                       |
| ------------ | ------------------------------------------------------ | ---------------------------------------------- |
| **性质**     | **编译器优化** (Elision)                               | **语言特性** (有明确的构造函数调用)            |
| **发生时机** | 编译期决定，是**构造位置的优化**                       | 运行期发生，是**对象内容的“搬运”**             |
| **开销目标** | **零开销**，完全避免构造和拷贝/移动                    | **低成本开销**，通常是拷贝指针、资源句柄等     |
| **要求**     | 编译器支持，代码结构符合优化条件（如直接返回局部变量） | 对象类型必须具有可访问的移动（或拷贝）构造函数 |
| **可观测性** | 被省略的拷贝/移动构造函数的**副作用不会被触发**        | 移动构造函数的**副作用会被触发**               |

## 4. 为什么NRVO能避免拷贝、提升性能？

**NRVO通过“构造即最终位置”来消除所有中间步骤。**

1. **无优化（最差情况，类型无法移动，Pod类型或删除了移动构造和移动赋值）**：
   - 在 `createObject` 函数栈帧中构造 `obj`。
   - `return obj;` 时，在函数返回的临时区域调用**拷贝构造函数**，从 `obj` 复制出一个新对象。
   - 函数返回后，调用处可能再调用一次**拷贝/移动构造函数**，将这个临时对象初始化目标变量。
   - **总计：可能2次额外构造。**
2. **使用移动语义（次优情况，非Pod类型实现移动构造和移动赋值）**：
   - 在 `createObject` 函数栈帧中构造 `obj`。
   - `return obj;` 触发**移动构造函数**，将 `obj` 的资源“转移”到返回的临时对象。
   - 调用处再通过一次**移动构造**初始化目标变量。
   - **总计：2次移动构造。** 避免了深拷贝，但仍有两次“搬运”操作和相关的指针更新。
3. **应用NRVO（最优情况）**：
   - 编译器**直接在调用者为返回值分配的内存地址上**（也就是最终目标变量的位置）构造 `obj`。
   - `return obj;` 语句**不产生任何实际的拷贝或移动操作**，仅仅是一个“名字绑定”或流程控制。
   - **总计：1次构造，零次拷贝/移动。** 这就是“零开销抽象”的完美体现。

**关键补充**：

- **优先级**：`NRVO/RVO` > `移动` > `拷贝`。
- **副作用**：由于拷贝/移动构造函数可能被省略，**它们的副作用（如打印日志）不会被执行**。这在设计类时需要留意。
- **强制优化**：在 **C++17** 标准中，**纯RVO（返回匿名临时对象）是强制要求编译器执行的**，而NRVO仍然是**非强制**的优化（但主流编译器在常见场景下都会做）。
- **如何帮助编译器**：编写简单、直接的返回语句（如 `return local_var;`），避免对返回变量进行复杂的条件分支返回不同的对象，这有助于编译器启用NRVO。

# `const指针规则`

定义为`const`的变量，不得转为非`const`指针来访问
但有几个特例，允许你去掉`const`：
...
1. `char*`可以访问任何类型的变量（便做**memcpy**)
2. **`unsigned int*`**可以访问`int`变量（带有unsigned修饰不影响)
3. `const int**`可以访问`int*`变量（二级指针带有const修饰不影响)
4. 结构体A如果含有int成员，那么可以`A*`访问int变量（允许通过结构体指针访问成员）
...
| 引用类型     | 绑定对象 | 自身是 | 语法             | 主要用途                         | 关键特性                           |
| :----------- | -------- | ------ | :--------------- | :------------------------------- | :--------------------------------- |
| **左值引用** | 左值     | 左值   | `T&`             | 别名、按引用传参修改值、避免拷贝 | 绑定左值，不可空，不可重绑定       |
| **右值引用** | 右值     | 左值   | `T&&`            | 移动语义、完美转发               | 绑定右值，延长临时对象             |
...
struct Str
{
  Str(std::string_view s) : s(s) { std::println("constructor called"); }
  Str(const Str& s) : s(s.s) { std::println("copy constructor called"); }
  ...
  ~Str() { std::println("destructor called {}", s); }
  std::string s;
};

int main()
{
  Str{ "123" }; // 纯右值，生命周期一行，即出了 ; 被调用析构
    
  Str&& s{ "123" }; // 延长纯右值的生命周期，它是左值，因为有名字s，更准确的来说是纯右值其实变为了将亡值，将亡值是泛左值也是右值，可以用一个右值引用进行接收
    
  Str&&   s2 = std::forward<Str>(s); // 转发
  Str&  s3 = s; // 引用别名
  // s s2 s3 都是同一份资源std::string "123" 的别名，他们并不持有资源，持有资源的是将亡值
  
  // 移动资源，此时 将亡值仍旧没有被析构，只是其内部的std::string的资产 123 被转移给了 s4，那么自己的资产就是空了 ""
  Str     s4 = std::move(s3);
  {
    Str&& s5 = std::move(s4); // 别名
  }// 资产是s4管理，出了} 并不会让 s4析构
    
  std::println("{}", s.s); //空，注意空不意味着对象没有，这里指的是空字符串
  std::println("{}", s2.s);//空
  std::println("{}", s3.s);//空
  std::println("{}", s4.s);//123
  
  return 0;
}
// 按照内存模型来，定义的顺序和析构顺序相反，符合“栈模型”，注意代码所有的变量都是栈变量，即使是指针也是在栈中，因为变量名字都是使用栈地址，仅仅是这个指针指向堆内存而已
// 先打印 析构 123
// 再打印 析构 "" 空字符串看不到
```
...
#define NARGS(...) NARGS_IMPL(__VA_ARGS__ __VA_OPT__(, ) 3, 2, 1, 0)
```
...
```
#define CHECK_GL x; check_gl_error(__FILE__, __LINE__, #x);
```

如果你使用CHECK_GL宏作用于条件分支，如：

```
if (!window) CHECK_GL(114514);
```
...
这样我们if的分支作用域就包裹不完全，所以我们使用do while包裹，保证会执行一次

```
#define CHECK_GL do {	\
x; check_gl_error(__FILE__, __LINE__, #x);	\
} while(0)
```

### 2. 为什么不用 `{}`而是 `do { ... } while(0)`

你想弄明白为什么在定义这类宏时，用单纯的 `{}` 包裹会有问题，而必须用 `do { ... } while(0)` 这种结构，核心是为了让宏在不同语法场景下（尤其是条件分支后）都能保持正确的执行逻辑和语法合法性。
...
if (!window) {
    114514;
    check_gl_error(__FILE__, __LINE__, "114514");
}; // 注意这里多出来的分号！
```

这会引发两个关键问题：
1. **语法错误（多余的分号）**：C/C++ 中，`if` 分支后如果是代码块 `{}`，结尾**不应该加分号**。但用户使用宏时习惯写 `CHECK_GL(...);`（带分号），展开后就会出现 `};`，虽然多数编译器能兼容，但这是不符合语法规范的写法，极端场景下可能触发编译警告/错误。
2. **逻辑陷阱（else 分支匹配问题）**：如果后续加 `else`，问题会更严重：
   ```c
   // 用户写的代码
   if (!window) CHECK_GL(114514);
   else printf("window is valid");
   ```
   展开后：
   ```c
   if (!window) {
   ...
   else printf("window is valid"); // else 没有对应的 if，直接编译报错！
   ```

#### (2) do while(0) 包裹的核心优势
改用 `do { ... } while(0)` 包裹宏：
```c
// 正确写法：do while(0) 包裹
...
当用户写 `if (!window) CHECK_GL(114514);` 时，展开后是：
```c
if (!window) do {
    114514;
    check_gl_error(__FILE__, __LINE__, "114514");
} while(0); // 分号是 do while 结构的合法结尾
```

这个写法完美解决了 `{}` 包裹的问题：
1. **语法合法性**：`do while(0)` 是一个完整的“单语句”结构，结尾加 `;` 是符合 C/C++ 语法的，不会出现多余分号的问题。
2. **逻辑完整性**：`do while(0)` 会被当作 `if` 分支下的**一个整体语句**，后续的 `else` 能正确匹配 `if`，不会出现“else 无对应 if”的错误。
3. **执行保证**：`while(0)` 确保这个循环只会执行一次，既保留了“包裹代码块”的作用，又不会有循环执行的额外开销（编译器会优化掉这个无效循环）。

#### (3) 总结
1. 单纯用 `{}` 包裹宏，在条件分支后使用时会因“多余分号”导致语法错误（尤其是搭配 `else` 时）；
2. `do { ... } while(0)` 是 C/C++ 宏定义的经典技巧，既能将多行代码包裹成“一个整体语句”，又能兼容用户使用宏时加 `;` 的习惯；
3. `while(0)` 保证代码只执行一次，且编译器会优化掉这个空循环，无性能损耗。

### 3. `()` 包裹宏参数

在上述提到`do { ... } while(0)`包裹宏防止`if else`因为分号`;`出错，但是此时给出的宏语句并不完善，完善的宏语句如下
...
* 核心是为了避免宏参数在不同传入场景下（比如多表达式、关键字、运算符优先级）出现语法或逻辑错误，确保参数始终被当作**一个完整的表达式**执行。

你想弄明白在 `do { ... } while(0)` 包裹的宏里，为什么要给参数 `x` 加上括号 `(x)`，核心是为了避免宏参数在不同传入场景下（比如多表达式、关键字、运算符优先级）出现语法或逻辑错误，确保参数始终被当作**一个完整的表达式**执行。
...
##### 场景1：传入“多表达式”参数（逗号/分号分隔）
如果传入的参数包含多个表达式（比如用逗号分隔），不加括号会破坏宏的执行逻辑：
```c
// 用户调用：传入两个表达式
CHECK_GL(glClear(GL_COLOR_BUFFER_BIT), glFlush());
```
宏展开后：
```c
do {
    glClear(GL_COLOR_BUFFER_BIT), glFlush();; // 多余分号，语法怪异
    check_gl_error(__FILE__, __LINE__, "glClear(GL_COLOR_BUFFER_BIT), glFlush()");
} while(0);
```
看似能执行，但如果参数里混有分号（比如用户误写）：
```c
// 错误调用（但语法上用户可能这么写）
CHECK_GL(1; 2; 3);
```
...
```c
do {
    1; 2; 3;; // 多层分号，虽然编译器可能兼容，但破坏了“参数是一个整体”的语义
    check_gl_error(__FILE__, __LINE__, "1; 2; 3");
} while(0);
```

##### 场景2：传入 `break`/`continue` 等关键字
这是**最关键的问题**：如果参数是 `break`/`continue`，不加括号会让关键字作用于外层循环（而非宏内部），导致逻辑完全错误。

举个例子，宏被用在循环里：
```c
// 外层循环
for (int i = 0; i < 10; i++) {
    CHECK_GL(break); // 期望：宏内部执行 break（但宏内无循环，应该报错/无效）
}
```
不加括号时，宏展开后：
```c
for (int i = 0; i < 10; i++) {
    do {
        break; // break 作用于外层的 for 循环！直接跳出整个 for，而非 do-while
        check_gl_error(__FILE__, __LINE__, "break");
    } while(0);
}
```
结果：`break` 跳过了 `check_gl_error` 的执行，直接跳出外层 `for` 循环，完全违背了宏的设计意图（先执行 `x`，再检查错误）。

##### 场景3：运算符优先级问题
如果传入的参数包含运算符（比如 `a + b * c`），不加括号可能因优先级导致计算错误（虽然这个场景在 GL 调用中少见，但属于宏定义的通用坑）：
```c
#define MUL(x) x * 2
// 调用
int res = MUL(1 + 2); // 期望：(1+2)*2=6，实际：1+2*2=5
```
同理，GL 相关的参数如果有运算符（比如 `GL_TRUE || GL_FALSE`），不加括号也会触发优先级问题。
...
括号 `()` 会将传入的内容包裹成**一个不可分割的表达式**：
- 如果传入多表达式（`1,2,3`），`(1,2,3)` 会被当作“逗号表达式”执行（先算 1、再算 2、最后算 3，结果为 3），但语法上是合法的“单个表达式”；
- 如果传入非法的多语句（`1;2;3`），`(1;2;3)` 会直接触发**编译错误**（括号内不允许分号分隔的多语句），提前暴露用户的错误调用，而非默默执行错误逻辑。

##### 2. 限制 `break`/`continue` 的作用域
当参数是 `break`/`continue` 时，括号会强制关键字作用于宏内部的 `do-while` 循环，而非外层循环：
```c
for (int i = 0; i < 10; i++) {
    CHECK_GL(break); // 宏展开后
}
...
for (int i = 0; i < 10; i++) {
    do {
        (break); // break 作用于内层的 do-while 循环（执行一次就终止）
        check_gl_error(__FILE__, __LINE__, "break"); // 这行不会执行！
    } while(0);
}
```
此时 `break` 只会跳出内层的 `do-while`（循环本就只执行一次），**不会影响外层的 for 循环**，符合“宏内部逻辑独立”的设计原则（即使 `check_gl_error` 没执行，也避免了意外跳出外层循环的严重问题）。

> 补充：如果用户本意是让 `break` 作用于外层循环，那传入 `break` 到宏里本身就是错误用法，括号的作用是“限制错误影响范围”，而非“允许错误用法”。

##### 3. 规避运算符优先级问题
括号会强制参数内的表达式优先计算，彻底解决优先级陷阱：
```c
#define MUL(x) (x) * 2
int res = MUL(1 + 2); // 展开为 (1+2)*2=6，符合预期
```
对于 GL 调用，比如 `CHECK_GL(glGetError() == GL_NO_ERROR)`，括号能确保整个判断表达式作为一个整体执行，不会因运算符优先级被拆分。

#### (3) 极端场景验证：传入多参数/复杂表达式
再看一个更复杂的场景，验证括号的必要性：
```c
// 用户调用：传入带运算符的表达式
CHECK_GL((glBindBuffer(GL_ARRAY_BUFFER, buf), glBufferData(GL_ARRAY_BUFFER, size, data, GL_STATIC_DRAW)));
```
宏展开后：
```c
do {
    ((glBindBuffer(GL_ARRAY_BUFFER, buf), glBufferData(GL_ARRAY_BUFFER, size, data, GL_STATIC_DRAW)));
    check_gl_error(__FILE__, __LINE__, "(glBindBuffer(GL_ARRAY_BUFFER, buf), glBufferData(GL_ARRAY_BUFFER, size, data, GL_STATIC_DRAW))");
} while(0);
```
- 外层括号 `(x)` 确保整个逗号表达式是一个整体；
- 即使传入的内容本身带括号，多层括号也不会触发语法错误（C/C++ 允许表达式外层加任意多层括号）。

如果不加外层括号，逗号表达式可能被拆分，或与宏内的其他代码（比如分号）冲突。

#### (4) 总结
1. **强制表达式完整性**：`(x)` 把参数包裹成单个表达式，避免多语句/多表达式导致的语法混乱，非法用法（如分号分隔）会直接编译报错，提前暴露问题；
2. **限制关键字作用域**：`break`/`continue` 等关键字只会作用于宏内部的 `do-while` 循环，不会意外影响外层循环/逻辑；
3. **规避优先级陷阱**：括号强制参数内的表达式优先计算，确保复杂表达式（带运算符）按预期执行；
4. **语法兼容性**：多层括号在 C/C++ 中合法，不会触发编译错误，是宏定义的通用最佳实践。

简单来说，`do { ... } while(0)` 解决了“代码块作为单语句”的语法问题，而 `(x)` 解决了“参数作为单表达式”的逻辑问题，两者结合才是健壮的宏定义写法。
...
前提是在源代码中已经定义了一个取消宏，我们才可以预处理进行开关开启
...
前提是函数签名和宏冲突，使用`()`包裹宏，因为宏的特点是如果宏后面不是跟着左括号`(`它是不会进行展开的

```C++
std::min(1,2); // 如果使用#include <windows.h> 并且没有编译传入flag -DNOMINMAX，也没有#undef，那么这行会报错
// 可以使用
(std::min)(1,2); // 能够正确得到标准库函数std::min，而不是宏
```

## `宏覆盖` push_macro/pop_macro

预处理阶段解析规则：宏名字冲突，采用的宏是预处理阶段最后找到的宏。

```C++
#define PI 3.14
#define PI 3.1415926
constexpr double pi = PI; // 3.1415926
可以使用push_macro、pop_macro预处理命令在预处理阶段进行改变宏定义。
一个示例（自定义宏保存，取消它的定义，获取三方库宏值，重定义自定义宏覆盖了第三方宏值，如果对调两个定义的顺序，打印值不一样，符合我们的解析规则，MSVC / GCC / Clang结果均是这样的）：
// 我们自己的PI定义
#define PI 3.14

// 在包含第三方头文件之前，保存当前PI的定义，并取消定义PI
#pragma push_macro("PI")
#undef PI

// 包含第三方头文件，此时第三方可以定义它自己的PI
#include "third_party.h" // PI 3.1415926

int main() {
    std::cout.setf(std::ios_base::boolalpha);
    constexpr double p1 = PI;
    // 包含完毕后，恢复我们自己的PI定义
#pragma pop_macro("PI")
    // 从此处开始，PI恢复为我们之前定义的值（3.14），此时PI 为3.14是第二次定义，在三方库定义PI之后
    constexpr double p2 = PI;
    std::cout << "PI: " << p1 << std::endl; // 3.1415926
    std::cout << "PI: " << p2 << std::endl; // 3.14
    std::cout << (p1 == p2) << std::endl;
    return 0;
}
```

合理的逻辑顺序应该能够解决大部分冲突需求

# `Contracts`契约

首先明确：**契约不是为了替代异常，而是为了更优雅地表达程序的“正确性约束”**，比如函数的前置条件、后置条件、不变式等，本质是“编程契约”——调用者和被调用者约定好的规则。

## 1. 契约的核心作用

- **显式表达约束**：将代码中的隐含规则（如“参数必须大于0”）直接写在函数接口处，提升代码可读性和可维护性。
- **分阶段检查**：支持“仅调试检查”“始终检查”“不检查”等不同级别，兼顾调试效率和运行时性能。
- **标准化错误处理**：替代手写的`assert`或`if (条件) throw`，提供统一的、可配置的约束检查机制。
- **辅助编译器优化**：编译器可基于契约的约束条件（如“参数非空”）进行更精准的优化。

## 2. 契约的核心语法（C++26草案）

C++26的契约主要通过`[[expects]]`（前置条件）、`[[ensures]]`（后置条件）、`[[assert]]`（通用断言）等属性实现，核心语法如下：

```cpp
#include <iostream>
#include <stdexcept>

// 示例1：前置条件（[[expects]]）——函数调用前必须满足的条件
int divide(int a, int b) [[expects: b != 0]] {
    return a / b;
}

// 示例2：带错误处理的前置条件（指定违反时的行为）
int safe_divide(int a, int b) 
    [[expects audit throw(std::invalid_argument("除数不能为0")): b != 0]] 
{
    return a / b;
}

// 示例3：后置条件（[[ensures]]）——函数执行后必须满足的条件
int square(int x) [[ensures audit: return >= 0]] {
    return x * x;
}

// 示例4：类的不变式（[[invariant]]）——对象生命周期内始终满足的条件
class PositiveNumber {
    int val;
public:
    PositiveNumber(int v) [[expects: v > 0]] : val(v) {}
    
    void set(int v) [[expects: v > 0]] {
        val = v;
    }
    
    // 不变式：val始终大于0
    [[invariant audit: val > 0]] void check() const {}
};
...
int main() {
    ...
    try {
        ...
        std::cerr << "safe_divide错误：" << e.what() << std::endl;
    }

    // 测试3：违反后置条件（square返回负数，实际不可能，仅示例）
    // try {
    //     std::cout << square(-5) << std::endl; // 输出25，满足后置条件
    // } catch (...) {
    //     std::cerr << "捕获square异常" << std::endl;
    // }
    ...
    try {
        ...
        std::cerr << "PositiveNumber构造失败" << std::endl;
    }
...
| 特性         | 契约（Contracts）                      | 异常（Exceptions）                     |
| ------------ | -------------------------------------- | -------------------------------------- |
| **核心目的** | 检查“程序正确性约束”（前置/后置条件）  | 处理“运行时错误”（如IO失败、内存不足） |
| **使用场景** | 验证调用者是否遵守接口规则（参数合法） | 处理不可避免的运行时异常情况           |
| **检查级别** | 可配置（仅调试、始终检查、不检查）     | 始终执行（除非禁用异常）               |
| **语义**     | 违反契约=程序逻辑错误（bug）           | 异常=运行时意外情况（非bug）           |

**关键结论**：

- 契约**不是替代异常**，而是补充：
  - 契约用于“断言程序逻辑的正确性”（比如“参数必须非空”），违反契约通常意味着代码有bug；
  - 异常用于“处理运行时不可预测的错误”（比如“文件打开失败”），这类错误即使代码无bug也可能发生。
- 契约可以和异常结合使用（如示例2中`throw(...)`），让契约违反时抛出指定异常。

**总结**

1. **核心作用**：C++26契约用于显式表达程序的“正确性约束”（前置/后置条件），提升代码可读性和可维护性，辅助编译器优化，而非替代异常。
2. **契约vs异常**：契约检查“逻辑正确性”（违反=bug），异常处理“运行时错误”（非bug），二者互补而非替代。
3. **执行行为**：契约违反时的行为由检查级别和`throw`指定——默认中断（`abort`），指定`throw`则抛出异常（程序不中断，可捕获），发布模式可关闭检查。

关键点：契约是“预防性的正确性检查”，异常是“补救性的错误处理”，在实际开发中应根据场景选择：参数合法性用契约，运行时错误用异常。

## 4. 契约违反时的执行行为（中断还是继续）

C++26的契约支持**3种检查级别**，不同级别对应不同的行为：

### 1. 默认级别（`default`）

- 行为：由实现定义（编译器决定），通常调试模式下中断（`abort`），发布模式下跳过检查。
- 示例：`[[expects: b != 0]]`（未指定级别）。

### 2. 审计级别（`audit`）

- 行为：始终检查，违反时的行为由`handler`指定（如抛出异常、中断）。
- 示例：`[[expects audit throw(...) : b != 0]]`（违反时抛出指定异常，程序不中断，可捕获处理）。

### 3. 强制级别（`axiom`）

- 行为：编译器假设条件始终成立（不检查），仅用于辅助优化（如“参数非空”）。
- 示例：`[[expects axiom: b != 0]]`（编译器优化时认为b≠0，违反时行为未定义）。

**核心规则**：

- 若契约未指定`throw(...)`，默认违反时**程序中断**（调用`std::abort`，无法捕获，直接退出）；
- 若契约指定`throw(异常类型)`，违反时**抛出指定异常**，程序不中断，可通过`try-catch`捕获处理；
- 发布模式下可通过编译器选项关闭契约检查（如`-fcontracts=off`），此时契约不执行，程序继续运行（但可能导致未定义行为）。

## 5. 契约的编译与运行配置（不代表最终设置）

不同编译器（如GCC、Clang）对C++26契约的支持需开启对应选项：

```bash
# GCC编译示例（需GCC 14+，C++26支持）
g++ -std=c++26 -fcontracts=on -o contract_demo contract_demo.cpp

# 配置契约检查级别
# -fcontracts=default：默认级别
# -fcontracts=audit：审计级别（始终检查）
# -fcontracts=axiom：强制级别（不检查，仅优化）
# -fcontracts=off：关闭所有契约检查


gitlab.com/lock3/gcc-new/-/wikis/contract-assertions
-fcontract-build-level=<off|default|audit>

g++ -std=c++2a -fcontracts -fcontract-build-level=audit
```

...
![image-20260114035457888](assets/image-20260114035457888.png)
...
* `thread`接收一个可以`invoke`的函数，然后后面接万能引用的参数包

* 如果可调用函数的参数是 `&T` 类型，例如`std::string`
  ...
  * ```C++
    ...
    int main() {
    	std::string str;
    	std::thread tr(foo, str);// 调用拷贝构造，我们想修改str将不会实现，使用std::ref包装
    	tr.join();
    }
    ```

  * 构造一个`std::string`，然后作为参数传递，实际上我们传入的str是一个被拷贝的对象，我们调用了拷贝构造函数

  * 解决使用`std::ref` `std::cref`进行包装
...
void simple_worker(int id) {
    for (int i = 0; i < 5; ++i) {
        std::cout << "Thread " << id << " equal " << std::this_thread::get_id() << " working: " << i << std::endl;
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
}
...
`std::thread`是标准库提供的线程库，但是各平台底层的类型不同，Windows底层是`HANDLE`，Linux底层是`pthread_t`，他们都可以通过调用`native_handle()`进行获取
...
int main() {
	std::thread t(foo);
	HANDLE handle = t.native_handle();	// Windows下获取句柄修改线程名字，VS可以通过线程查看名字变化，Linux是pthread_setname_np
	SetThreadDescription(handle, L"MyThread"); // 注意字符串是wstring
    auto id = t.get_id;
	std::println("{}", id);
	t.join();
	return 0;
...
- **std::promise**: 承诺在未来提供某个值，用于线程共享数据传输的，作为invoke的参数传递promise，未来某一时刻设置
- **std::future**: 用于获取承诺的值

| 类/方法                   | 描述                                                         |
| ------------------------- | ------------------------------------------------------------ |
| `std::promise<T>`         | 创建一个承诺，可以在未来设置一个类型为 T 的值                |
| `promise.set_value()`     | 设置承诺的值，只能设置一次                                   |
...
| `future.valid()`          | 调用get之后future将被销毁，只能获取一次，valid检查 future 是否有效 |
...
| `std::launch::deferred` | 延迟执行，直到调用 future.get()，同步 |
| 无策略参数              | 由实现决定执行方式              |
...
### （5）其他组件future_status、future_error、future_errc

- **std::future_status**: 表示等待操作的结果（ready, timeout, deferred）
  - deferred：延迟执行意味着你不能对其等待，wait_for，wait_until无效
  - ready：任务的状态已就绪，可以调用get获取结果
  - timeout：如果在wait_for、wait_until等待期间共享状态仍未就绪且超时
...
```C++
int opeation(std::promise<int>& data) {
	using namespace std::chrono_literals;
	auto f = data.get_future();
	try {
		std::println("[task] Waiting for data");
		auto count = f.get();
		std::println("[task] Got data: {}", count);
		int sum = 0;
		...
		std::println("[task] Exception: {}", e.what());
		return -1;
	}
}
int main() {
	...
	std::future<int> result = std::async(std::launch::async, opeation, std::ref(data));
	std::this_thread::sleep_for(1s);
	std::cout << "[main] Sending data" << std::endl;
	try {
		data.set_value(10);
		if (result.valid()) {
			auto sum = result.get();
			std::println("[main] Result: {}", sum);
		}
	}
	catch (const std::exception& e) {
		data.set_exception(std::current_exception());
		// or data.set_exception(std::make_exception_ptr(e))
	}
	return 0;
}
```
...
- **第一个实现（基于 `std::atomic_bool`）**：（适用于长临界区，4个线程内最优，大于4个线程std::mutex最优）
...
- **第二个实现（基于 `std::atomic_flag`）**：（适用于短临界区）
...
{
  int64_t counter = 0;
  int64_t data [1024];  // 更大的数据结构以增加缓存压力
...
// 3. 高强度工作负载 - 在锁内执行更多操作
static void BM_StdMutexIntensiveWorkload(benchmark::State& state)
{
  ...
  // 减少本地缓冲，增加全局访问
  const int iterations_per_lock = state.range(0);
  ...
  for (auto _ : state) {
    ...
    // 在锁内执行高强度操作
    for (int i = 0; i < iterations_per_lock; i++) {
      // 复杂的计算操作
      int index                = dist(rng) % 1024;
      shared_data.data [index] = shared_data.data [index] * 3 + 7;

      // 使用分支增加复杂度
      if (shared_data.data [index] % 2 == 0) {
        shared_data.data [(index + 1) % 1024] += shared_data.data [index];
      }
    }
  ...
  // 设置基准测试指标
  state.SetItemsProcessed(state.iterations() * iterations_per_lock);
  state.SetBytesProcessed(state.iterations() * iterations_per_lock *
                          sizeof(int64_t));
}

// 4. SpinMutex 高强度工作负载
static void BM_SpinMutexIntensiveWorkload(benchmark::State& state)
{
  static SharedData shared_data;
...
// 5. 微秒级锁竞争测试
static void BM_StdMutexMicroContention(benchmark::State& state)
{
  static std::atomic<int64_t> counter { 0 };
  static std::mutex           mutex;

  // 模拟真实场景中的短暂锁持有
  const int work_cycles =
      100 + (state.thread_index() % 50);  // 不同线程不同工作量
  ...
  for (auto _ : state) {
    ...
    // 模拟锁外的一些工作，但比锁内少
    volatile int small_work = 0;
    for (int i = 0; i < 10; i++) {
      small_work += i;
    }
    benchmark::DoNotOptimize(small_work);
  }

  state.SetItemsProcessed(state.iterations());
}

static void BM_SpinMutexMicroContention(benchmark::State& state)
{
  static std::atomic<int64_t> counter { 0 };
  static SpinMutex            mutex;
...
// 高强度工作负载测试 - 调整锁内操作数量
BENCHMARK(BM_StdMutexIntensiveWorkload)
    ->Arg(10)    // 每个锁10次操作
    ->Arg(100)   // 每个锁100次操作
    ->Arg(1000)  // 每个锁1000次操作
    ->ThreadRange(1,
                  std::thread::hardware_concurrency() * 2)  // 最多2倍CPU核心数
    ->UseRealTime()
    ->MeasureProcessCPUTime()
    ->Unit(benchmark::kMillisecond);

BENCHMARK(BM_SpinMutexIntensiveWorkload)
    ->Arg(10)
    ->Arg(100)
    ->Arg(1000)
    ->ThreadRange(1, std::thread::hardware_concurrency() * 2)
    ->UseRealTime()
    ->MeasureProcessCPUTime()
    ->Unit(benchmark::kMillisecond);

// 微秒级锁竞争测试
BENCHMARK(BM_StdMutexMicroContention)
    ->ThreadRange(1, std::thread::hardware_concurrency() * 2)
    ->UseRealTime()
    ->MeasureProcessCPUTime()
    ->Unit(benchmark::kMicrosecond);

BENCHMARK(BM_SpinMutexMicroContention)
    ->ThreadRange(1, std::thread::hardware_concurrency() * 2)
    ->UseRealTime()
    ->MeasureProcessCPUTime()
    ->Unit(benchmark::kMicrosecond);
...
### （7）自定义自旋锁、std::mutex、MSVC自旋锁 性能测试

使用C++20自定义自旋锁性能最差，标准实现自旋锁性能最好，MSVC共享指针自旋锁次之，std::mutex/无锁原子操作最次、无锁原子操作仅在1线程性能最快，随着线程数增加可能比std::mutex慢
...
// 模拟MSVC共享自旋锁的简单实现
struct Shared_ptr_spin_lock { // class to manage a spin lock for shared_ptr atomic operations
    void lock() { // lock the spin lock
        std::_Lock_shared_ptr_spin_lock();
    }

    void unlock() noexcept { // unlock the spin lock
        std::_Unlock_shared_ptr_spin_lock();
    }
};
...
bool test_accuracy(const std::string& name, int thread_count, int iterations_per_thread) {
    MutexType mutex;
    Counter counter;
    std::vector<std::thread> threads;
    ...
    long long expected = thread_count * iterations_per_thread;
    long long actual = counter.get();
    bool correct = (expected == actual);

    std::cout << name << " 准确性测试:" << std::endl;
    std::cout << "  线程数: " << thread_count
        << ", 每线程迭代: " << iterations_per_thread << std::endl;
    std::cout << "  期望值: " << expected
        << ", 实际值: " << actual
        << ", 结果: " << (correct ? "✓ 正确" : "✗ 错误") << std::endl;
    std::cout << std::endl;
...
// 性能测试函数
template<typename MutexType>
void benchmark_mutex(const std::string& name, int thread_count, int iterations_per_thread) {
    MutexType mutex;
    Counter counter;
    std::vector<std::thread> threads;
    ...
    std::cout << name << " 性能测试结果:" << std::endl;
    std::cout << "  线程数: " << thread_count
        << ", 每线程迭代: " << iterations_per_thread
        << ", 总操作: " << counter.get()
        << " [" << (correct ? "✓" : "✗") << "]" << std::endl;
    std::cout << "  总时间: " << duration << " μs"
        << " (" << duration / 1000.0 << " ms)" << std::endl;
    std::cout << "  平均每次锁操作: " << static_cast<double>(duration) / counter.get() << " μs" << std::endl;
    std::cout << "  吞吐量: " << (counter.get() * 1000000.0 / duration) << " 操作/秒" << std::endl;
    std::cout << std::endl;
}

// 无锁情况下的基准测试（用于对比）
void benchmark_baseline(const std::string& name, int thread_count, int iterations_per_thread) {
    ...
    // 验证结果准确性
    long long expected = thread_count * iterations_per_thread;
    long long actual = counter.load();
    bool correct = (expected == actual);

    std::cout << name << " 测试结果:" << std::endl;
    std::cout << "  线程数: " << thread_count
        << ", 每线程迭代: " << iterations_per_thread
        << ", 总操作: " << counter.load()
        << " [" << (correct ? "✓" : "✗") << "]" << std::endl;
    std::cout << "  总时间: " << duration << " μs"
        << " (" << duration / 1000.0 << " ms)" << std::endl;
    std::cout << "  平均每次操作: " << static_cast<double>(duration) / counter.load() << " μs" << std::endl;
    std::cout << "  吞吐量: " << (counter.load() * 1000000.0 / duration) << " 操作/秒" << std::endl;
    std::cout << std::endl;
}

// 专门测试数据竞争的函数
void test_data_race() {
    std::cout << "=== 数据竞争测试 ===" << std::endl;
    ...
    // 测试无锁情况下的数据竞争
    {
        ...
        long long expected = thread_count * iterations;
        long long actual = counter.get();
        std::cout << "  期望: " << expected << ", 实际: " << actual
            << ", 数据丢失: " << (expected - actual)
            << " (" << ((expected - actual) * 100.0 / expected) << "%)" << std::endl;
    }
...
int main() {
    ...
    // 测试参数
    const int accuracy_iterations = 100000;  // 准确性测试的迭代次数
    const int performance_iterations = 1000000;  // 性能测试的迭代次数

    // 先测试准确性
    std::cout << "=== 准确性验证测试 ===" << std::endl;
    std::vector<int> test_threads = { 1, 2, 4 };

    bool all_correct = true;
    for (int threads : test_threads) {
        all_correct &= test_accuracy<SpinMutex>("自定义自旋锁", threads, accuracy_iterations);
        all_correct &= test_accuracy<Shared_ptr_spin_lock>("MSVC风格自旋锁", threads, accuracy_iterations);
        all_correct &= test_accuracy<std::mutex>("std::mutex", threads, accuracy_iterations);
    }

    if (!all_correct) {
        std::cout << "警告: 部分测试发现正确性问题!" << std::endl;
    }
    else {
        std::cout << "所有准确性测试通过! ✓" << std::endl;
    }
    std::cout << std::endl;
    ...
    for (int threads : thread_counts) {
        std::cout << "=== " << threads << " 个线程测试 ===" << std::endl;

        // 无锁基准测试
        benchmark_baseline("无锁原子操作", threads, performance_iterations);

        // 各种锁的性能测试
        benchmark_mutex<SpinMutex>("自定义自旋锁", threads, performance_iterations);
        benchmark_mutex<Shared_ptr_spin_lock>("MSVC风格自旋锁", threads, performance_iterations);
        benchmark_mutex<std::mutex>("std::mutex", threads, performance_iterations);
    ...
    // 额外测试：高竞争场景
    std::cout << "=== 高竞争场景测试 (16线程) ===" << std::endl;
    benchmark_mutex<SpinMutex>("自定义自旋锁", 16, performance_iterations / 10);
    benchmark_mutex<Shared_ptr_spin_lock>("MSVC风格自旋锁", 16, performance_iterations / 10);
    benchmark_mutex<std::mutex>("std::mutex", 16, performance_iterations / 10);

    // 最终总结
    std::cout << "=== 测试总结 ===" << std::endl;
    std::cout << "✓ 添加了准确性验证" << std::endl;
    std::cout << "✓ 添加了数据竞争演示" << std::endl;
    std::cout << "✓ 所有测试都包含正确性检查" << std::endl;
    std::cout << "✓ 性能测试结果标记了正确性状态" << std::endl;
...
The following cannot be coroutines
•main
• constructors
• destructors
 constexpr and consteval functions
• functions declared with placeholder types (auto or
C++20 concepts)
• functions with variable numbers of arguments

![image-20260114053253416](assets/image-20260114053253416.png)



![image-20260114062138163](assets/image-20260114062138163.png)
...
int main() {
    ...
    std::barrier sync_point(#pragma omp parallel num_threads(num_threads));
...
# MSVC 下 std::mutex 与 std::atomic 底层结构深度解析

在 C++ 标准库中，`std::mutex` 和 `std::atomic` 是并发编程的核心组件，二者在 MSVC 编译器下的底层实现差异显著——前者基于复杂的结构体实现，侧重功能完整性；后者依托轻量的原子变量，侧重高性能。本文基于 MSVC 内部头文件 `__msvc_threads_core.hpp` 的源码，详细解析二者的底层结构设计。

## 一、核心背景与基础类型定义

MSVC 对 C++ 线程库的实现基于 Windows 系统原生接口，先定义了一系列底层基础类型，作为 `std::mutex` 和 `std::atomic` 的实现基石：

### 1.1 核心基础类型（extern "C" 块内定义）

这些类型是 MSVC 线程库的底层抽象，屏蔽了 Windows 系统 API 的细节：

| 类型名         | 用途               | 具体定义                                                     |
| -------------- | ------------------ | ------------------------------------------------------------ |
| `_Thrd_id_t`   | 线程 ID 类型       | 本质是 `unsigned int`，对应 Windows 线程 ID                  |
| `_Thrd_t`      | 线程标识符结构体   | 包含 `void* _Hnd`（Windows 线程句柄 HANDLE）和 `_Thrd_id_t _Id`（线程 ID） |
| `_Smtx_t`      | 轻量锁指针类型     | 本质是 `void*`，作为原子操作/轻量锁的核心载体                |
| `_Thrd_result` | 线程操作返回值枚举 | 包含 `_Success`/`_Nomem`/`_Timedout`/`_Busy`/`_Error`，标识线程/锁操作结果 |
| `_Mtx_t`       | 互斥锁指针类型     | 指向 `_Mtx_internal_imp_t` 结构体的指针                      |
| `_Cnd_t`       | 条件变量指针类型   | 指向 `_Cnd_internal_imp_t` 结构体的指针                      |

### 1.2 编译环境适配

MSVC 会根据编译架构（32/64 位）调整结构体大小，核心宏定义：

- `_WIN64`（64 位）：临界区存储大小 `_Critical_section_size = 64`，条件变量存储大小 `_Cnd_internal_imp_size = 72`
- 32 位：临界区存储大小 `_Critical_section_size = 36`，条件变量存储大小 `_Cnd_internal_imp_size = 40`

## 二、std::atomic 底层实现（轻量核心）

`std::atomic` 是原子变量的核心实现，其底层依赖 MSVC 定义的 `_Stl_critical_section` 结构体，整体设计极致轻量化：

### 2.1 底层核心结构体

```cpp
struct _Stl_critical_section {
    void* _Unused       = nullptr; // 兼容旧 ABI 的占位字段（原虚函数指针）
    _Smtx_t _M_srw_lock = nullptr; // 核心原子锁指针，本质是 void*
};
```

### 2.2 关键特性

1. **内存占用极轻**：仅包含两个指针字段（`_Unused` 为占位，实际核心是 `_M_srw_lock`），64 位下仅 16 字节（32 位下 8 字节），且实际有效字段仅 `_M_srw_lock`（1 个指针，8/4 字节）；
2. **依赖硬件原子指令**：`_M_srw_lock` 最终映射到 Windows 的 SRWLOCK（轻量读写锁），底层直接调用 CPU 原子指令（如 x86 的 `LOCK` 前缀、CAS 指令），无需陷入内核（竞争时除外）；
3. **无额外状态**：仅维护锁的核心指针，无线程 ID、计数等额外信息，是“最小化”的同步原语。

## 三、std::mutex 底层实现（功能完整）

`std::mutex` 底层基于 `_Mtx_internal_imp_t` 结构体实现，侧重功能完整性，内存占用显著高于 `std::atomic`：

### 3.1 底层核心结构体

```cpp
struct _Mtx_internal_imp_t {
    // 临界区存储大小（64位=64字节，32位=36字节）
    static constexpr size_t _Critical_section_size = _WIN64 ? 64 : 36;

    int _Type{}; // 锁类型标识（如普通互斥锁、递归锁等）
    union {
        _Stl_critical_section _Critical_section{}; // 基础临界区结构
        // 对齐的存储缓冲区，适配不同架构的临界区大小
        _STD _Aligned_storage_t<_Critical_section_size, alignof(void*)> _Cs_storage;
    };
    long _Thread_id{}; // 持有锁的线程 ID，用于递归锁/死锁检测
    int _Count{}; // 锁的计数（递归锁的重入次数，普通 mutex 为 0/1）
};
```

### 3.2 关键特性

1. **内存占用较大**：64 位下整体大小约 80 字节（`_Type`(4) + 联合体(64) + `_Thread_id`(4) + `_Count`(4)，含内存对齐），32 位下约 48 字节，远大于 `std::atomic`；
2. **功能丰富**：
   - `_Type`：区分锁类型（普通互斥锁、递归锁、超时锁等）；
   - `_Thread_id`：记录当前持有锁的线程 ID，防止同一线程重复加锁（普通 mutex 会检测，递归锁允许）；
   - `_Count`：记录锁的重入次数，支持递归锁（`std::recursive_mutex`）的实现；
3. **依赖内核调度**：竞争时会通过 Windows 的 `CriticalSection` 或 `SRWLOCK` 进入内核态，将线程放入等待队列，触发上下文切换，开销高于 `std::atomic`。

## 四、std::condition_variable 补充（关联 mutex）

MSVC 同时定义了条件变量的底层结构 `_Cnd_internal_imp_t`，用于配合 `std::mutex` 实现等待/唤醒逻辑：

```cpp
struct _Stl_condition_variable {
    void* _Unused = nullptr; // 兼容旧 ABI 的占位字段
    void* _Win_cv = nullptr; // 指向 Windows 原生条件变量（CONDITION_VARIABLE）
};

struct _Cnd_internal_imp_t {
    static constexpr size_t _Cnd_internal_imp_size = _WIN64 ? 72 : 40;
    union {
        _Stl_condition_variable _Stl_cv{}; // 基础条件变量结构
        _STD _Aligned_storage_t<_Cnd_internal_imp_size, alignof(void*)> _Cv_storage;
    };
};
```

条件变量依赖 `std::mutex` 实现，底层映射到 Windows 的 `CONDITION_VARIABLE`，进一步体现 `std::mutex` 的“重量级”设计。

## 五、核心对比（MSVC 下）

| 特性              | std::atomic                            | std::mutex                                    |
| ----------------- | -------------------------------------- | --------------------------------------------- |
| 底层核心类型      | `_Stl_critical_section`（仅 1 个指针） | `_Mtx_internal_imp_t`（复杂结构体）           |
| 内存占用（64 位） | ~8 字节（有效字段）                    | ~80 字节（完整结构体）                        |
| 核心字段          | 仅 `_M_srw_lock`（轻量锁指针）         | `_Type`/`_Thread_id`/`_Count`/临界区          |
| 底层依赖          | CPU 原子指令（用户态为主）             | Windows CriticalSection/SRWLOCK（内核态兜底） |
| 功能侧重          | 单个变量的原子操作（计数、标志位）     | 临界区保护（复杂代码块、递归、条件等待）      |
| 性能              | 极高（无上下文切换）                   | 中等（竞争时触发上下文切换）                  |

## 六、总结

1. **std::atomic**：MSVC 下底层基于 `_Stl_critical_section` 的 `_M_srw_lock` 指针实现，仅占用少量内存（核心字段为 1 个指针），直接调用硬件原子指令，适用于单个变量的原子操作（如计数器、标志位）；
2. **std::mutex**：底层基于 `_Mtx_internal_imp_t` 结构体实现，包含锁类型、线程 ID、重入计数等字段，内存占用约 80 字节（64 位），功能完整但开销更高，适用于复杂临界区的保护；
3. **设计逻辑**：二者均基于 Windows 系统原语（SRWLOCK/CriticalSection），但 `std::atomic` 聚焦“轻量原子性”，`std::mutex` 聚焦“完整同步功能”，体现了“性能”与“功能”的权衡。

## 七、杂谈

原子变量和锁，**两者都需要硬件和操作系统（以及高级语言/库）的协同工作，但它们的分工和侧重点不同。**

**硬件对原子指令的支持，是实现锁的必要非充分条件。** 没有原子指令，就建不起可靠的多核并发锁。

我们可以用一个类比来理解：

*   **硬件** 提供了最基础的“砖块”和“工具”（如特殊的原子指令）。
*   **操作系统** 用这些“砖块”和“工具”建造了更完整、功能更丰富的“房子”（如系统调用、调度器支持）。
*   **编程语言库** 则为程序员提供了进入“房子”的便捷“门”和“窗户”（如 `pthread_mutex_t`, `std::atomic`）。

下面我们来详细拆解：

### 1. 锁的实现

锁是一个更高层次的同步抽象。它的实现是一个 **“硬件指令 + 操作系统内核支持” 的经典组合**。

*   **硬件基础（关键）**：
    *   硬件（CPU）必须提供某种**原子操作指令**，这是构建一切锁的基石。最常见的指令是 **CAS** 或 **TSL**。
    *   这些指令保证在一个指令周期内，完成“读-改-写”内存的操作，不会被其他处理器或线程打断。
    *   硬件还提供了**内存屏障**或**内存排序**指令，确保多核CPU下的操作顺序符合预期。

*   **操作系统内核的实现（核心）**：
    操作系统利用硬件提供的原子指令，在内核中实现各种锁（如互斥锁、读写锁、自旋锁）。
    1.  **用户空间尝试**：当线程试图获取锁时，首先会在用户空间尝试一次快速的原子操作（例如，用CAS尝试将锁变量从0设为1）。如果成功，则开销很小，这被称为 **“快速路径”**。
    2.  **系统调用介入（慢路径）**：如果获取失败（锁已被占用），线程就会通过**系统调用**进入内核。
        *   内核可以将该线程放入该锁的**等待队列**，并将其状态置为“睡眠”。
        *   当锁被释放时，内核会从等待队列中唤醒一个或多个线程。
        *   内核负责处理进程/线程调度，确保锁的公平性，防止饥饿。

*   **给程序员提供的接口**：
    *   **操作系统API**：例如，POSIX线程库中的 `pthread_mutex_lock()` / `pthread_mutex_unlock()`。调用这些函数最终会触发上述的内核机制。
    *   **编程语言库**：例如，Java中的 `synchronized` 关键字、`ReentrantLock` 类；C++中的 `std::mutex`。它们通常是对操作系统原生锁API的封装。

**小结：锁 = 硬件的原子指令（基础） + 操作系统的调度和队列管理（功能主体） + 语言/库的API（接口）。**

### 2. 原子变量的实现

原子变量的目标比锁更“轻量”，它保证对单个变量的读写、修改操作是不可分割的。它的实现 **更偏重于硬件直接支持**。

*   **硬件基础（绝对核心）**：
    *   CPU直接提供了针对内存操作的**原子指令**。例如：
        *   `x86` 平台的 `LOCK` 指令前缀（用于 `ADD`, `XCHG`, `CMPXCHG` 等），可以锁定总线或缓存行，确保原子性。
        *   `ARM` 平台的 `LDREX`/`STREX` 指令对，通过独占监视器实现原子操作。
    *   现代CPU对对齐的、长度合适（通常是1, 2, 4, 8字节）的简单类型变量的读写，本身可能就是原子的。

*   **操作系统和编译器的角色**：
    *   **编译器**：当你在C++中使用 `std::atomic<int>` 或在Java中使用 `AtomicInteger` 时，编译器会识别这些操作，并生成对应的底层**原子机器指令**，而不是普通的内存读写指令。
    *   **操作系统**：对于原子变量的支持，操作系统层面的直接介入较少。主要职责是保证虚拟内存到物理内存的映射是有效的，以及提供必要的内存屏障原语。原子操作本身主要在用户空间由CPU指令完成，**不需要陷入内核**（这是它比锁快得多的关键原因）。
    *   **语言运行时库**：提供原子变量的高级抽象和内存序选择（如 `memory_order_relaxed`）。

**小结：原子变量 = 硬件的原子指令（绝对主力） + 编译器的特殊代码生成 + 语言/库的API封装。操作系统主要提供底层内存和屏障支持，但不直接管理其等待逻辑。**

### 对比表格

| 特性                 | **锁**                                         | **原子变量**                                     |
| :------------------- | :--------------------------------------------- | :----------------------------------------------- |
| **核心提供方**       | **操作系统内核**（基于硬件指令）               | **硬件CPU指令**                                  |
| **主要目的**         | 保护**一大段**临界区代码，管理复杂状态         | 保证对**单个变量**的读写/修改操作不可分割        |
| **是否需要内核介入** | **是**（在竞争时，需要系统调用进行线程调度）   | **通常否**（在用户空间由CPU指令直接完成）        |
| **开销**             | 较高（涉及系统调用、上下文切换）               | 很低（接近普通变量操作，但可能有缓存同步开销）   |
| **功能**             | 功能强大（可睡眠、可设置优先级继承、读写锁等） | 功能单一（仅变量原子性，常配合无锁算法）         |
| **适用场景**         | 保护复杂的共享数据结构或代码块                 | 实现计数器、标志位、或作为构建无锁数据结构的基础 |

### 总结

你可以这样理解：

*   如果你想保护一个“房间”（临界区），让一个人进去后别人不能进，你会用**锁**。这需要管理员（**操作系统**）来协调谁在等、谁下一个进。
*   如果你只想确保大家能安全、有序地在一个“小本子”（单一变量）上签名，你会用**原子变量**。这只需要一支设计好的、一次只能一个人用的“特殊笔”（**硬件指令**）即可。

因此，**硬件是地基，提供了实现原子性的物理可能；操作系统在地基上盖了“锁”这座功能齐全的大楼；而原子变量更像是硬件直接提供的一件精密的便携工具。**
...
{
    // 经过测试递归条件小于32 使用插入排序高效于 (size <= 1) return
    if (size <= 32) {

...
#### directory_iterator 和 file_type
...
int main()
{
    ...
    // 递归文件夹
    fs::path currentPath{dirPath};
    for (const auto& dir_entry : fs::directory_iterator{currentPath}) {
		std::println("{}", dir_entry.path().filename());
        // 特定文件类型
        switch(const auto& p = dir_entry.path(); dir_entry.status().type())
        {
        case: fs::file_type::directory: std::println("[Dir]: {}", p.string()); break;
        case: fs::file_type::regular: std::println("[file]: {}, size{}", p.string(), dir_entry.file_size()); break;
		}
	}
    ...
    std::partition(dir_entries.begin(), dir_entries.end(), [](const auto& de)
    {
...
![image-20251105231733947](assets/image-20251105231733947.png)

1. **std::cout**：标准输出流，用于输出一般信息。它是缓冲的，意味着输出可能会被缓存直到缓冲区满或遇到换行符等才会刷新。通常用于正常程序输出（给用户看的）。
2. **std::cerr**：标准错误流，用于输出错误信息。它是无缓冲的，意味着输出会立即刷新，确保错误信息立即显示。通常用于错误消息和紧急情况。
3. **std::clog**：标准日志流，用于输出日志信息。它是缓冲的，但通常与标准错误流关联（即同样输出到标准错误设备，如控制台）。它用于记录日志，因为缓冲可以提高性能，但请注意，由于缓冲，日志信息可能不会立即显示（给开发者或系统管理员看的）。

...
int main() {
    ...
    // 15. 修改键值
    std::map<int, string> modMap {{1, "one"}, {2, "two"}, {3, "three"}};
    printMap(modMap, "提取前的map");
    auto e = modMap.extract(2);
    printMap(modMap, "提取后的map");
    e.key() = 4;
    modMap.insert(std::move(e));
    // m == { { 1, "one" }, { 3, "three" }, { 4, "two" } }
    printMap(modMap, "访问被修改键值的map");

    // 16. 使用insert_or_assign方法
    cout << "\n=== 使用insert_or_assign方法 ===" << endl;
    map<int, string> ioamap = {{1, "one"}, {2, "two"}};
    printMap(ioamap, "初始ioamap");
    
    // 插入新元素
    auto res1 = ioamap.insert_or_assign(3, "three");
    cout << "插入键3: " << (res1.second ? "插入成功" : "覆盖已有") << endl;
    printMap(ioamap, "插入键3后");
    
    // 覆盖已有元素
    auto res2 = ioamap.insert_or_assign(2, "TWO");
    cout << "插入键2: " << (res2.second ? "插入成功" : "覆盖已有") << endl;
    printMap(ioamap, "覆盖键2后");
    
    // 17. 带提示位置的插入
    cout << "\n=== 带提示位置的插入 ===" << endl;
    map<int, string> hintMap = {{1, "one"}, {3, "three"}, {5, "five"}};
    printMap(hintMap, "初始hintMap");
    
    // 19. 使用try_emplace方法
    cout << "\n=== 使用try_emplace方法 ===" << endl;
    map<string, Person> personMap;
    
    // 使用try_emplace原地构造，避免不必要的拷贝/移动
    auto templace_res1 = personMap.try_emplace("Alice", 25, "Alice Smith");
    cout << "插入Alice: " << (templace_res1.second ? "成功" : "失败") << endl;
    
    auto templace_res2 = personMap.try_emplace("Bob", 30, "Bob Johnson");
    cout << "插入Bob: " << (templace_res2.second ? "成功" : "失败") << endl;
    
    // 尝试插入已存在的键
    auto templace_res3 = personMap.try_emplace("Alice", 26, "Alice Brown");
    cout << "再次插入Alice: " << (templace_res3.second ? "成功" : "失败") << endl;
    
    cout << "personMap内容:" << endl;
...
14. **排序数组**：通过**std::vector随机访问迭代器**的特性，以及**std::multiset双向迭代器**的特性，将std::vector构造std::multiset，然后在使用`std::copy`或者`std::move`遍历multiset在vector.data开始逐个拷贝赋值或者移动得到排序std::vector
...
### [Ⅲ. map](https://142857.red/book/stl_map/#map_8)
...
int main() {
    ...
    printMap(m, "提取前的map");
    auto e = m.extract(2); // 返回一个node_type 类型，类似unique_ptr，能够自动管理内存的智能指针，可以std::move
    printMap(m, "提取后的map");
    e.key() = 4;
    m.insert(std::move(e));
    // m == { { 1, "one" }, { 3, "three" }, { 4, "two" } }
...
2. **添加元素**：
   ...
   - `insert()` 返回一个 pair，包含迭代器和布尔值表示是否插入成功
     ...
     insert和[]相同点，如果不存在就都会插入；不同点，如果存在，那么insert不会覆盖，[]会覆盖

3. **访问元素**：

   - `[]` 运算符（<u>建议写时使用</u>）: 通过键访问值，如果键不存在会创建新元素

     **[]运算符的坑：如果使用其来查找读取不到一个元素它不会报错，而是返回0，并且写入这个找不到的数据**

   - `at()`（<u>建议读时使用</u>）: 通过键访问值，如果键不存在会抛出异常
...
12. **使用emplace方法**：使用 `emplace()` 方法直接构造并插入键值对，**对于map来说，emplace所谓的原地构造无效，因为他是一个pair类型，仍旧是构造pair，然后调用移动，但是对于vector、set这样单个元素来说，那么emplace将会很有效**
...
15. **修改键值**：使用`extract()`方法，返回的是一个特殊的”智能指针“，意味着可以花括号RAII析构哦，指针指向pair<const K,V>将节点从红黑树总提取，红黑树将它”剥离“，所有权交给使用者，操作单个节点修改键，然后使用`insert()`方法再插入该节点

16. `pair<iter, bool> insert_or_assign(const K&, V)`：如果K不存在插入创建，如果K存在就写入覆盖，注意bool返回的意思，插入创建true、写入覆盖false；而insert时插入成功或插入失败的意思

    - **使用场景优势一：相较于`[]运算符`，性能更好**

      **覆盖旧的值时，都是调用 移动或者拷贝赋值，而再创建新的KV时，**

      **`insert_or_assign`少了一次的调用V的默认构造函数，并且他是调用拷贝构造或者移动构造，**

      **`[]运算符`则是调用V的默认构造，然后`operator=`赋值或移动**

    - 使用场景优势二：如果V没有默认构造函数，那么`[]运算符`编译会报错

    - 注意：取代 么`[]运算符` <u>仅限于 写入操作</u>，像使用 container[key]++这样的无元素时，返回V 0，然后++进行 统计计数 的优雅写法无法替代

17. `iterator insert(const_iterator pos, const pair<K, v>& kv)`：带提示位置的插入，我们如果知道一个插入目标大概的位置那么，将会非常高效，如果提示位置不准确那么也没什么损失，建议构造数据的时候时有序的

18. `try_emplace`：C++17引入的`try_emplace`才是真正的有用，`insert`总是不可避免调用移动构造，而**`try_emplace`则是原地构造**，它的参数是，第一个参数是key键，第二个参数**往后的多个参数是value值的 <u>构造函数的参数个数</u>**（注意类型匹配）

    - m.try_emplace("key", 18, "Alice"); 一次构造，注意没有花括号
    - m.insert({"key", Person{18, "Alice"}});一次构造，两次移动（移动构造pair给参数pair，移动参数pair给map）
    - m.insert(make_pair({"key", Person{18, "Alice"}}));一次构造，三次移动（移动构造pair给参数MakePair，移动MakePair参数pair，移动参数pair给map）

19. `merge`：C++17新增`merge`方法，m.merge(& source)会将source的节点移动给m，如果key冲突就不移动保留节点再source，和insert的区别是，insert不管有没有冲突节点，insert的数据都保持不变，相当于仅是for了source的资源拷贝一份给map
...
- 需要快速通过键查找值的场景
- 需要存储配置、字典等数据的场景
- map 轻松实现去重 + 动态排序，如何处置重复的键随你决定：
  - map 的 insert 只接受第一次出现的键-值对。
  - map 的 insert_or_assign 只保留最后一次出现的键-值对。
...
- 插入map，尽量适用`try_emplace`（一次构造）（C++17起）替代`insert和[]`（两次构造），`try_emplace`相较于`insert和[]`减少1次默认构造开销，并且try_emplace相较于emplace，在重复插入元素的时候无任何开销，而emplace插入重复元素也会默认构造有负载的对象
...
> 详细 透明仿函数见 [](通用工具库.md)   `<functional>`第5点的介绍
...
### Ⅰ. stack（默认`deque` 的设计避免了 `vector` 的"重新分配风暴"）

`std::stack` 是 C++ 标准库中的容器适配器，它提供了栈的功能（后进先出，LIFO）。stack 基于其他容器（如 deque、vector 或 list）实现，默认使用 deque 作为底层容器，因为deque扩容之后不会将已有的元素移动，而是保留这“块”内存，vector则需要移动所有元素，所以stack不提供reserve功能，如果你确定内存使用大小，建议使用vector进行预留空间，模拟栈功能。以下是 stack 的增删改查操作示例：
...
![image-20251023030042386](assets/image-20251023030042386.png)
...
int main() {
    ...
    // 15. 字符串拼接 append，调用的重载不一样，没有std::string历史遗留问题
     using namespace std::literals;
     std::string s{ "hello," };
     s.append("world", 3); 
     std::cout << s << "\n"; // 输出hello,wor，如果要截断后半部分，因为是char*，我们可以指针+3
     s = "hello,"s;
     s.append("world"s, 3);
     std::cout << s << "\n";// 输出hello,ld，对于字符串，我们可以"world"s.resize(3)得到前半部分
    
    // 16. insert也有两个版本是这样的
    
    // 17. C++20新增 starts_with、ends_with
    std::string file{"text.txt"};
    bool _1 = file.starts_with("text");
    bool _2 = file.ends_with("text");
...
15. **append字符串追加问题**：
    - 请注意char*、std::string重载的两个版本区别
16. **insert字符串插入问题**：
    - 请注意char*、std::string重载的两个版本区别
17. **starts_with、ends_with**
    - 查找开头和结尾的chars是否等价于xxx
...
#### std::string空基类优化——C++20新增一个类似属性[[no_unique_address]]

![image-20251023041433880](assets/image-20251023041433880.png)

![image-20251023041100664](assets/image-20251023041100664.png)

![image-20251023041039007](assets/image-20251023041039007.png)

![image-20251023041449401](assets/image-20251023041449401.png)

![image-20251023041217546](assets/image-20251023041217546.png)

#### std::string的小字符串优化

当字符串长度小于16字节时，最大15字节，因为还需要预留一个字节给C语言 '\0' 擦屁股。

分配的内存在栈上

![image-20251023042007570](assets/image-20251023042007570.png)

![image-20251023042119917](assets/image-20251023042119917.png)

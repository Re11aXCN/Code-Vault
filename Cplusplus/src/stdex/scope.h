...
class [[nodiscard]] scope_guard_base {
  struct dummy {};
  using exception_count_t =
      std::conditional_t<ScopeGuardType != scope_guard_type::exit, int, dummy>;

public:
  template <typename F>
    requires ExitFunction<F>
  explicit scope_guard_base(F &&func) noexcept(
      std::is_nothrow_constructible_v<EF, F>)
      : exit_function_(std::forward<F>(func)), active_(true) {
  ...
  scope_guard_base &operator=(scope_guard_base &&other) noexcept(
      std::is_nothrow_move_assignable_v<EF> &&
      std::is_nothrow_move_constructible_v<EF>) {
    if (this != &other) {
      if (active_)
        execute_if_needed();
...
class [[nodiscard]] scope_exit
    : public detail::scope_guard_base<EF, detail::scope_guard_type::exit> {
...
class [[nodiscard]] scope_fail
    : public detail::scope_guard_base<EF, detail::scope_guard_type::fail> {
...
class [[nodiscard]] scope_success
    : public detail::scope_guard_base<EF, detail::scope_guard_type::success> {
...
// 工厂函数（便于使用）
// ============================================================================
template <typename EF> [[nodiscard]] auto make_scope_exit(EF &&ef) {
  return scope_exit<std::decay_t<EF>>(std::forward<EF>(ef));
}

template <typename EF> [[nodiscard]] auto make_scope_fail(EF &&ef) {
  return scope_fail<std::decay_t<EF>>(std::forward<EF>(ef));
}

template <typename EF> [[nodiscard]] auto make_scope_success(EF &&ef) {

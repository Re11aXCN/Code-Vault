...
int nthUglyNumber_v4(int n)
{
  ...
  for (; i + 7 < n; i += 8) {
    ...
    for (int j = 0; j < 8; j++) {
      ...
      ugly [i + j] = next2 < next3 ? (next2 < next5 ? next2 : next5)
...
int nthUglyNumber_v6(int n)
{
  ...
  for (; i + 15 < n; i += 16) {
    ...
    for (int j = 0; j < 16; j++) {
      ...
      // 手动内联min函数
      int min_val = (next2 < next3) ? ((next2 < next5) ? next2 : next5)
                                    : ((next3 < next5) ? next3 : next5);
...
bool verifyCorrectness()
{
  ...
  for (int i = 1; i <= 100; i++) {
    ...
    if (r1 != r2 || r1 != r3 || r1 != r4 || r1 != r5 || r1 != r6) {
      std::cout << "Error at n=" << i << ": " << r1 << "," << r2 << "," << r3
                << "," << r4 << "," << r5 << "," << r6 << std::endl;
      return false;
    }
  }
  std::cout << "All tests passed! All versions produce same results."
            << std::endl;
  return true;
}
...
void performanceTest()
{
  ...
  std::cout << "\nPerformance Test: n=" << n << ", iterations=" << iterations
            << std::endl;
  std::cout << "==========================================" << std::endl;
  ...
  for (size_t i = 0; i < versions.size(); i++) {
    ...
    std::cout << i + 1 << ". " << versions [i].first << ": "
              << versions [i].second << " μs (" << speedup
              << "x slower than fastest)" << std::endl;
  }
}

...
public:
  ...
  static void sort(T* data, int left, int right, int badAllowed = 0)
  {
    ...
    // 递归排序
    if (pivotPos - left > 16 && right - pivotPos > 16) { --badAllowed; }

    sort(data, left, pivotPos - 1, badAllowed);
    sort(data, pivotPos + 1, right, badAllowed);
  }

  static void sort(T* data, int n) { sort(data, 0, n - 1, int(log2(n)) * 2); }
};
...
void hoare_quick_sort(T* data, int left, int right, Compare comp)
{
  ...
  while (true) {
    do {
      ++i;
    } while (i < right && comp(data [i], pivot));
    do {
      --j;
    } while (j > left && comp(pivot, data [j]));
...
private:
  ...
  static void
  test_sort_function(SortFunction<T> sort_func, const std::string& name,
                     ...
                     std::vector<TestResult>& results)
  {
    ...
    bool correct =
        is_sorted(test_data.data(), static_cast<int>(test_data.size()));
...
public:
  ...
  {
    std::cout << "=== Quick Sort Benchmark (Data Size: " << data_size
              << ") ===" << std::endl;
    ...
    std::vector<std::string>    case_names = { "Sorted (Ascending)",
                                               "Sorted (Descending)",
                                               "Partially Sorted", "Random" };
    ...
    for (int case_idx = 0; case_idx < 4; ++case_idx) {
      ...
      for (size_t i = 0; i < sort_functions.size(); ++i) {
        test_sort_function(sort_functions [i], function_names [i], test_data,
                           original_data, results);
      }
      test_sort_function<1>(sort_functions [0], "std::sort", test_data,
                            original_data, results);
      ...
      std::cout << "Fastest: " << fastest->name << " (" << fastest->time_ms
                << " ms)" << std::endl;
    }
  }
};
...
#pragma GCC unroll 8
    ...
    void hoare_quick_sort(RanIter start, RanIter end, Compare comp)
    {
        ...
        auto pivot_val = *mid;
        // 优化分区循环的边界检查，如果不移动枢轴 需要额外处理枢轴位置
要跳过枢轴本身 auto pivot_pos = std::prev(right); // 倒数第二个位置
        std::iter_swap(mid, pivot_pos); // 将枢轴放到倒数第二个位置

        // left(i) ≤ pivot_pos(j) ≤ right,
已有序情况下，我们从i的next，j的prev开始 auto i = left, j = pivot_pos; while
(true) { do { ++i; } while (i < right && comp(*i, pivot_val)); // 找比枢轴大 do
{ --j; } while (j > left && comp(pivot_val, *j)); // 找比枢轴小 if (i >= j)
break; std::iter_swap(i, j);
        }

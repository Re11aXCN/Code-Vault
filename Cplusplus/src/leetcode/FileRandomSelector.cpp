...
bool FileRandomSelector::ParseArgs(int argc, char* argv[]) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " --suffix=ext --filter=[file1,file2] --rand-num=N" << std::endl;
        return false;
    }
            ...
            if (entry.is_regular_file()) {
                ...
                if (!_suffix.empty()) {
                    if (filename.length() <= _suffix.length() ||
                        filename.substr(filename.length() - _suffix.length() - 1) != "." + _suffix) {
                        continue;
                    }
                }

                // 检查过滤列表
                if (std::find(_filter_list.begin(), _filter_list.end(), filename) != _filter_list.end()) {
                    continue;
                }

                // 检查历史记录
                if (_history_files.find(filename) != _history_files.end()) {
                    continue;
                }
...
bool FileRandomSelector::loadHistory() {
    ...
    while (std::getline(file, line)) {
        ...
        if (line.front() == '[' && line.back() == ']') {
            ...
            current_entry.timestamp = line.substr(1, line.length() - 2); // 去掉方括号
            in_entry = true;
        }
        else if (in_entry) {
            // 如果是文件名行
...
bool FileRandomSelector::saveHistory() {
    ...
    // 写入头信息
    file << "totalFile=" << _total_files << ", cacheNum=" << _cached_files << std::endl;
...
std::vector<std::string> FileRandomSelector::randomSelectFiles(const std::vector<std::string>& files) {
    if (files.empty()) {
        return {};
    }
    ...
    std::vector<std::string> selected(shuffled.begin(), shuffled.begin() + select_count);

    return selected;
}

void FileRandomSelector::parseFilterList(const std::string& filterStr) {
    if (filterStr.length() < 2 || filterStr.front() != '[' || filterStr.back() != ']') {
        return;
    }
    ...
    if (!lastItem.empty()) {
        _filter_list.push_back(lastItem);
    }
}

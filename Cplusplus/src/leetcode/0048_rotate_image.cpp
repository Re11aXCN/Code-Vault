...
public:
                      ...
                      const std::vector<std::vector<int>>& rotated)
  {
    ...
    for (int i = 0; i < n; ++i) {
      for (int j = 0; j < n; ++j) {
        if (rotated [i][j] != original [n - 1 - j][i]) { return false; }
      }
    }
    return true;
  }
  ...
  copyMatrix(const std::vector<std::vector<int>>& matrix)
  {
    return matrix;
  }
  ...
  void runBenchmark()
  {
    ...
    for (int n : sizes) {
      ...
      start        = std::chrono::high_resolution_clock::now();
      for (int i = 0; i < 2; ++i)
        rotateMatrixDirectUnrolled(matrix3);
      end = std::chrono::high_resolution_clock::now();
      auto duration3 =
          std::chrono::duration_cast<std::chrono::microseconds>(end - start);
      ...
      start        = std::chrono::high_resolution_clock::now();
      for (int i = 0; i < 2; ++i)
        rotateMatrixTransposeUnrolled(matrix4);
      end = std::chrono::high_resolution_clock::now();
      auto duration4 =
          std::chrono::duration_cast<std::chrono::microseconds>(end - start);
      bool valid4 = verifyRotation(original, matrix4);

      std::cout << "直接旋转法: " << duration1.count()
                << " μs, 正确性: " << (valid1 ? "✓" : "✗") << "\n";
      std::cout << "转置+逆序法: " << duration2.count()
                << " μs, 正确性: " << (valid2 ? "✓" : "✗") << "\n";
      std::cout << "直接旋转(循环展开): " << duration3.count()
                << " μs, 正确性: " << (valid3 ? "✓" : "✗") << "\n";
      std::cout << "转置+逆序(循环展开): " << duration4.count()
                << " μs, 正确性: " << (valid4 ? "✓" : "✗") << "\n";
    }
  }
};

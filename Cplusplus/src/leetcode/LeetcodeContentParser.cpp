...
private:
  ...
  std::string _parseWeekday(const std::string& weekday_str)
  {
    ...
    std::vector<std::string> weekdays = { "星期一", "星期二", "星期三",
                                          "星期四", "星期五", "星期六",
                                          "星期日" };
...
public:
  ...
  // 公有方法：大写命名
  LeetcodeContentParser(const std::string& current_date)
  ...
  std::vector<LeetcodeRecord> ParseContent(const std::string& content)
  {
    ...
    while (std::getline(stream, line)) {
      ...
      // 检查是否是日期行
      if (line.find("小时前") != std::string::npos || line == "昨天" ||
          _isWeekday(line) ||
          line.find("月") != std::string::npos &&
              line.find("日") != std::string::npos) {
        // 如果已经有完整的记录，保存它
  ...
  std::string GetCurrentDate() const { return _current_date; }
};

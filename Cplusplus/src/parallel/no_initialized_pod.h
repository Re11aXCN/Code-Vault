...
public:
    ...
    no_initialized_pod(no_initialized_pod&& p) : _M_t(std::move(p._M_t)) {}
    no_initialized_pod& operator=(const no_initialized_pod& p) { _M_t = p._M_t; return *this; }
    no_initialized_pod& operator=(no_initialized_pod&& p) { _M_t = std::move(p._M_t); return *this; }

    no_initialized_pod(const _Ty& t) : _M_t(t) {}
    no_initialized_pod(_Ty&& t) : _M_t(std::move(t)) {}
    no_initialized_pod& operator=(const _Ty& t) { _M_t = t; return *this; }
    no_initialized_pod& operator=(_Ty&& t) { _M_t = std::move(t); return *this; }

...
namespace avx {
    ...
    public:
        ...
        static size_t filter(const float* input, size_t count, float threshold, float* output);

        template<ComparisonOperator Operator>
        static size_t filterPlain(const float* input, size_t count, float threshold, float* output);

        /**
         * @brief 过滤大于阈值的元素
         */
        static size_t filterGreaterThan(const float* input, size_t count, float threshold, float* output);

        /**
         * @brief 过滤小于阈值的元素
         */
        static size_t filterLessThan(const float* input, size_t count, float threshold, float* output);
        static size_t filterPlainLessThan(const float* input, size_t count, float threshold, float* output);

        /**
         * @brief 过滤等于阈值的元素
         */
        static size_t filterEqual(const float* input, size_t count, float threshold, float* output);

    private:
        template<int ComparisonCode>
        static size_t filterImpl(const float* input, size_t count, float threshold, float* output);
    
        template<int ComparisonCode>
        static size_t filterPlainImpl(const float* input, size_t count, float threshold, float* output);
    };
...
static void avx_filter_test(const std::vector<float>& scores) {
    ...
    tbb::parallel_for(
        ...
        [&](tbb::blocked_range<size_t> r) {
            ...
            std::pmr::vector<NoInitializedPod<float>> local_bad_scores{ &pool };
#else
            std::vector<NoInitializedPod<float>> local_bad_scores;
#endif
            local_bad_scores.resize(r.size());
#if 1
            size_t n = avx::AVXFilter::filterLessThan((float*)scores.data() + r.begin(), r.size(), 60, (float*)local_bad_scores.data());
#else
            size_t n = 0;
...
#pragma omp critical
    ...
    */
    ...
    if (has_invalid_score) {
        ...
        size_t end = (invalid_index + 5 < bad_scores.size()) ? invalid_index + 5 : bad_scores.size();
        std::cout << "附近的值: ";
        ...
        if (bad_scores.size() == expected_bad_count) {
            ...
            std::cout << "⚠️  数量不一致: 过滤出 " << bad_scores.size()
                << " 个分数，但预期有 " << expected_bad_count << " 个" << std::endl;
        }
    }
}
...
static void avx_filter_plain_test(const std::vector<float>& scores) {
    ...
    tbb::parallel_for(
        ...
        [&](tbb::blocked_range<size_t> r) {
            ...
            local_bad_scores.resize(r.size());
#if 1
            size_t n = avx::AVXFilter::filterPlainLessThan((float*)scores.data() + r.begin(), r.size(), 60, (float*)local_bad_scores.data());
#else
            size_t n = 0;
            for (size_t i = r.begin(); i != r.end(); ++i) {
                float score = scores[i];
